...
trait CDCReaderImpl extends DeltaLogging {
  ...
  protected def getDeletedAndAddedRows(
    ...
    finalAddFiles.foreach { case (fileVersion, addFiles) =>
    ...
    finalRemoveFiles.foreach { case (fileVersion, removeFiles) =>
      val commitInfo = versionToCommitInfo.get(fileVersion.version)
      dfAddsAndRemoves.append(
        scanIndex(
          spark,
  ...
  def processDeletionVectorActions(
    ...
    finalReplaceAddFiles.foreach { case (fileVersion, addFiles) =>
      val commitInfo = versionToCommitInfo.get(fileVersion.version)
      dfs.append(
        scanIndex(
          spark,
    ...
    // see explanation in [[generateFileActionWithInlineDv]].
    finalReplaceRemoveFiles.foreach { case (fileVersion, removeFiles) =>
      val commitInfo = versionToCommitInfo.get(fileVersion.version)
      dfs.append(
        scanIndex(
          spark,
