...
case class DeltaSource(
      ...
      deltaLog.getChangeLogFiles(startVersion, options.failOnDataLoss).flatMapWithClose {
        case (version, filestatus) =>
          if (filestatus.getLen <
            ...
            val addFiles =
              if (filterCommit(actions.exists(isBlindAppendCommit))) {
               verifyStreamHygieneAndFilterAddFiles(actions, version, verifyMetadataAction)
              } else Seq.empty
            ...
            val isBlindAppendCommit = try {
              verifyStreamHygiene(fileIterator.map(Action.fromJson), version, verifyMetadataAction)
            } finally {
              fileIterator.close()
            }
            fileIterator = if (filterCommit(isBlindAppendCommit)) {
              deltaLog.store.readAsIterator(filestatus.getPath, deltaLog.newDeltaHadoopConf())
            } else Iterator.empty.toClosable
  ...
  private def filterCommit(isBlindAppendCommitExist: Boolean) =
    if (onlyAppends) isBlindAppendCommitExist else true

  private def isBlindAppendCommit(a: Action): Boolean = a match {
    case commitInfo: CommitInfo => commitInfo.isBlindAppend.contains(true)
    case _: Action => false
  }
  ...
  protected def verifyStreamHygiene(
    ...
    actions.foreach {
      ...
      case commitInfo: CommitInfo =>
        skipCommit = commitInfo.isBlindAppend.contains(true)
      case _ => ()
    }
