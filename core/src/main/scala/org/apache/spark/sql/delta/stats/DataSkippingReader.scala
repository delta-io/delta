...
/** Trait representing a class that can generate [[DeltaScan]] given filters, limits, etc. */
...
trait DataSkippingReader
  ...
  private def constructLiteralInListDataFilters(a: Expression, values: Seq[Any]):
  ...
  private def constructDataFilters(dataFilter: Expression):
    ...
    // turn means that neither `a` nor `b` could evaulate to TRUE for any row the file might
  ...
  protected def verifyStatsForFilter(referencedStats: Set[StatsColumn]): Column = {
    ...
    // caller will negate the expression we return. In case a stats column is NULL, `NOT(expr)` must
    // return `TRUE`, and without these NULL checks it would instead return `NOT(NULL)` => `NULL`.
  ...
  def getFilesAndNumRecords(
      df: DataFrame): Iterator[(AddFile, java.lang.Long)] with Closeable = {
    val implicits = spark.implicits
    import implicits._
    val filesAndNumRecordsDataset = df.withColumn("numRecords", $"stats.numRecords")
      .withColumn("stats", nullStringLiteral)
      .select(struct($"*").as(SingleAction.addFileEncoder), $"numRecords".as[java.lang.Long])


    val resultIterator = filesAndNumRecordsDataset.collect().iterator
    new Iterator[(AddFile, java.lang.Long)] with Closeable {
      override def hasNext: Boolean = resultIterator.hasNext
      override def next(): (AddFile, lang.Long) = resultIterator.next()
      override def close(): Unit = {}
    }
  }

  private def pruneFilesByLimit(df: DataFrame, limit: Long): ScanAfterLimit = {
    val withNumRecords = {
    ...
    val filesAfterLimit = try {
      ...
      while (iter.hasNext && rowsToScan < limit) {
        val file = iter.next
        if (file._2 == null) { // this file has no stats, ignore for now
          bytesToIgnore += file._1.size
          filesToIgnore += file._1
        } else {
          rowsToScan += file._2.toLong
      ...
      if (rowsToScan < limit && filesToIgnore.nonEmpty) {
    ...
    val rowSize = if (rowsUnknown) None else Some(rowsToScan)
    ScanAfterLimit(filesAfterLimit.toSeq, Some(bytesToScan), rowSize)
  }
