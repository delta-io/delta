...
public class UCCatalogManagedCommitter implements Committer {
  ...
  public CommitResponse commit(
    ...
    logger.info("[{}] Attempting to stage + commit to UC table at version {}.", ucTableId, commitMetadata.getVersion());

    // TODO: log and enforce / do logic with the table type

    final String stagedCommitFilePath = CommitUtils.getStagedCommitFilePath(
        commitMetadata.getDeltaLogDirPath(), commitMetadata.getVersion());

    try {
      logger.info("[{}] Attempting to write staged commit file to {}", ucTableId, stagedCommitFilePath);
      engine
          .getJsonHandler()
          .writeJsonFileAtomically(
              ...
              finalizedActions,
              false /* overwrite */);
      logger.info("[{}] Successfully wrote staged commit file to {}", ucTableId, stagedCommitFilePath);
    } catch (IOException ex) {
      throw new CommitFailedException(
          true /* retryable */,
          false /* conflict */,
          "Failed to write staged commit file",
          ex);
    }

    final FileStatus kernelFileStatus;
    try {
      logger.info("[{}] Attempting to get written file status {}", ucTableId, stagedCommitFilePath);
