...
<Aside type="note">
  This feature is available in Delta Lake ?? and above.
</Aside>

Catalog-managed tables introduce a `catalogManaged` reader-writer table feature that changes how Delta Lake discovers and accesses tables. With this feature enabled, the catalog becomes the source of truth for commits instead of relying on filesystem PUT-if-absent primitives.
...
Filesystem-based access to catalog-managed tables is not supported. Delta clients must discover and access these tables through the managing catalog, not by direct path-based access.
</Aside>
...
You can enable catalog-managed commits on new tables.
