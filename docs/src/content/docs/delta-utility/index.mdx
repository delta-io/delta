...
## Show Delta table partitions

You can list the partitions of a partitioned Delta table using `SHOW PARTITIONS`.

Unlike Spark's native `SHOW PARTITIONS`, Delta returns one typed column for each partition key instead of a single `key=value/...` string. You can also provide an optional `PARTITION (...)` clause to filter the returned partitions.

See [Configure SparkSession](/delta-batch/#configure-sparksession) for steps to enable support for SQL commands in Apache Spark.

<Tabs syncKey="code-examples">
  <TabItem label="SQL">
  
    ```sql
    SHOW PARTITIONS eventsTable;
    ```
    ...
    ```sql
    SHOW PARTITIONS eventsTable PARTITION (date = '2024-01-01');
    ```
    ...
    ```sql
    SHOW PARTITIONS delta.`/data/events/`;
    ```
