...
Unlike Spark's native `SHOW PARTITIONS`, Delta returns one typed output column per partition column instead of a single `key=value/...` string. You can also provide an optional `PARTITION (...)` clause to filter the returned partitions.
