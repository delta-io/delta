...
Change data feed only tracks `UPDATE`, `DELETE`, and `MERGE` operations.
Simple `WRITE` operations are not tracked by CDF.
</Aside>
...
<Aside type="note" title="Operations tracked">
  Change data feed DOES track:
  - `UPDATE` - Generates both preimage (before) and postimage (after) records
  - `DELETE` - Generates delete records  
  - `MERGE` - Generates appropriate insert, update, and/or delete records based on the merge logic
  
  Change data feed DOES NOT track:
  - `WRITE` operations
</Aside>
