...
Delta Spark is a library for reading and writing Delta tables using Apache Spark™. For most read and write operations on Delta tables, you can use Apache Spark reader and writer APIs. For examples, see [Table batch reads and writes](/apache-spark-connector/table-batch-reads-and-writes/) and [Table streaming reads and writes](/apache-spark-connector/table-streaming-reads-and-writes/).

However, there are some operations that are specific to Delta Lake and you must use Delta Lake APIs. For examples, see [Table utility commands](/apache-spark-connector/table-utility-commands/).
