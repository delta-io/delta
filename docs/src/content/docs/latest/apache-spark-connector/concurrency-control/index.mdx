...
- For supported [storage systems](/latest/apache-spark-connector/storage-configuration), multiple writers across multiple clusters can simultaneously modify a table partition and see a consistent snapshot view of the table and there will be a serial order for these writes.
...
The following table describes which pairs of write operations can conflict. Compaction refers to [file compaction operation](/latest/apache-spark-connector/best-practices/#compact-files) written with the option `dataChange` set to `false`.
