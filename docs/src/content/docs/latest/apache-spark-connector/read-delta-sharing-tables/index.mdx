...
description: Learn how to read shared Delta tables using Delta Sharing protocol
---
...
Delta Sharing data source supports most of the options provided by Apache Spark DataFrame for performing reads through [batch](/latest/apache-spark-connector/table-batch-reads-and-writes), [streaming](/latest/apache-spark-connector/table-streaming-reads-and-writes), or [table changes (CDF)](/latest/apache-spark-connector/change-data-feed) APIs on shared tables. Delta Sharing doesn't support writing to a shared table. Please refer to the [Delta Sharing Repo](https://github.com/delta-io/delta-sharing/blob/main/README.md) for more details. Please follow the [quick start](https://github.com/delta-io/delta-sharing?tab=readme-ov-file#quick-start) to leverage the Delta Sharing python connector to discover the shared tables.

For Delta Sharing reads on shared tables with advanced Delta Lake features such as [Deletion Vectors](/latest/apache-spark-connector/what-are-deletion-vectors) and [Column Mapping](/latest/apache-spark-connector/delta-column-mapping), you need to enable integration with Apache Spark DataSourceV2 and Catalog APIs (since delta-sharing-spark 3.1) by setting the same configurations as Delta Lake when you create a new `SparkSession`. See [Configure SparkSession](/latest/apache-spark-connector/table-batch-reads-and-writes/#configure-sparksession).
...
The `timestamp_expression` and `version` share the same syntax as [Delta](/latest/delta-batch#timestamp-and-version-syntax).

## Read Table Changes (CDF)

Once the data provider turns on CDF on the original Delta Lake table and shares it with history through Delta Sharing, the recipient can query CDF of a Delta Sharing table similar to [CDF of a Delta table](/latest/delta-change-data-feed).
...
Delta Sharing Streaming supports the following functionalities in the same way as Delta Streaming: [Limit input rate](/latest/apache-spark-connector/table-streaming-reads-and-writes/#limit-input-rate), [Ignore updates and deletes](/latest/apache-spark-connector/table-streaming-reads-and-writes/#ignore-updates-and-deletes), [Specify initial position](/latest/apache-spark-connector/table-streaming-reads-and-writes/#specify-initial-position)
...
Please remember to set the spark configurations mentioned in [Configure SparkSession](/latest/apache-spark-connector/table-batch-reads-and-writes/#configure-sparksession) in order to read shared tables with Deletion Vectors and Column Mapping.

| Read Table Feature | Available since version |
| --- | --- |
| [Deletion Vectors](/latest/apache-spark-connector/what-are-deletion-vectors) | 3.1.0 |
| [Column Mapping](/latest/apache-spark-connector/delta-column-mapping) | 3.1.0 |
| [Timestamp without Timezone](https://spark.apache.org/docs/latest/sql-ref-datatypes.html) | 3.3.0 |
| [Type widening (Preview)](/latest/apache-spark-connector/delta-type-widening) | 3.3.0 |
