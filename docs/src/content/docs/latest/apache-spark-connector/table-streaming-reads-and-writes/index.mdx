...
For many Delta Lake operations on tables, you enable integration with Apache Spark DataSourceV2 and Catalog APIs (since 3.0) by setting configurations when you create a new `SparkSession`. See [Configure SparkSession](/latest/apache-spark-connector/table-batch-reads-and-writes/#configure-sparksession).
...
<Aside type="note">
  ...
  `logRetentionDuration` [configuration](/latest/apache-spark-connector/table-batch-reads-and-writes/#data-retention) and
  the stream lags in processing, Delta Lake processes the data corresponding to
  the latest available transaction history of the source table but does not fail
...
<Image src={import('./delta-initial-snapshot-data-drop.png')} alt="Initial Snapshot" />
...
The same `DataFrameWriter` options can be used to achieve the idempotent writes in non-Streaming job. For details [Idempotent writes](/latest/apache-spark-connector/table-batch-reads-and-writes/#idempotent-writes).
