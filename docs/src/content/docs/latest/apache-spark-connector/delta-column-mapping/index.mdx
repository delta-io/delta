...
description: Learn how to use column mapping in Delta Lake tables
---
...
For more examples, see [Rename columns](/latest/apache-spark-connector/table-batch-reads-and-writes/#rename-columns).
...
[Drop columns](/latest/apache-spark-connector/table-batch-reads-and-writes/#drop-columns).
...
- Enabling column mapping on tables might break downstream operations that rely
  ...
  [Change data feed limitations for tables with column mapping enabled](/latest/apache-spark-connector/change-data-feed/#change-data-feed-limitations-for-tables-with-column-mapping-enabled).
