...
Some users in the community have reported that Snowflake, unlike [Trino](https://trino.io/docs/current/connector/delta-lake.html) or [Spark](/latest/apache-spark-connector/table-batch-reads-and-writes/), is not using [Delta statistics](/latest/apache-spark-connector/optimizations/#data-skipping) to do data skipping when reading Delta tables. Due to this bug, Snowflake may read a lot of unnecessary parquet files resulting in poor query performance and increased API call requests from cloud providers.
...
As a workaround, you can enable [Delta UniForm](/latest/apache-spark-connector/universal-format-uniform/) to generate Iceberg metadata and read these tables as Iceberg tables from Snowflake.
</Aside>
