 ...
 *
 * Additionally, augments StreamingRelationV2 output (from `.table("delta.`path`")`) with CDC
 * columns when readChangeFeed=true, because RelationResolution creates StreamingRelationV2 with
 * `table.columns.toAttributes` which doesn't include CDC columns.
 *
...
class ApplyV2Streaming(
   ...
   * Preserves the original table column order to match DSv1's CDCReader.cdcReadSchema().
   */
