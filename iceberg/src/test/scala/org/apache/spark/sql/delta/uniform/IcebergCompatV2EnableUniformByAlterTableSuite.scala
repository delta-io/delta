...
trait IcebergCompatV2ParquetTestUtilsBase extends WriteDeltaHMSReadIceberg {
   ...
   */
  protected def createVanillaDeltaTableWithDV(id: String): Unit = {
    write(
      s"""
   ...
   * @param loc the table location.
   */
  protected def createVanillaDeltaTableWithoutDV(id: String): Unit = {
    write(
      s"""
         | CREATE TABLE $id (id INT, name STRING, date TIMESTAMP)
         | USING DELTA
   ...
   * @param loc the table location.
   */
  protected def createIcebergCompatV1Table(id: String): Unit = {
    write(
      s"""
         | CREATE TABLE $id (id INT, name STRING, date TIMESTAMP)
         | USING DELTA
   ...
   * @param loc the table location.
   */
  protected def createLiquidDeltaTable(id: String): Unit = {
    write(
      s"""
         | CREATE TABLE $id (id INT, name STRING, date TIMESTAMP)
         | USING DELTA
   ...
   * @param loc the table location.
   */
  protected def createDeltaTableWithNestedTypesAndColumnMapping(id: String): Unit = {
    write(
      s"""
         | CREATE TABLE $id (
         |   id INT,
...
class IcebergCompatV2EnableUniformByAlterTableSuite extends IcebergCompatV2ParquetTestUtilsBase {
  /**
  ...
  test("Deletion Vectors Needs To Be Disabled Before Enabling IcebergCompatV2 By ALTER TABLE") {
    withTempTableAndDir { case (id, _) =>
      ...
      val ex = intercept[DeltaUnsupportedOperationException](enableIcebergCompatV2ByAlterTable(id))
      assertResult(
