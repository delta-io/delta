...
public class ParquetStatsReader {
    ...
    private static DataFileStatistics mergeStats(
        ...
        for (Column statsColumn : statsColumns) {
            ...
            if (!stats.isPresent() || !isStatsSupportedDataType(columnType)) {
                continue;
            }
    ...
    private static Literal decodeMinMaxStat(
        ...
        if (dataType instanceof ByteType) {
            ...
            return Literal.ofDate((Integer) statValue); // stats are stored as epoch days
    ...
    private static DataType getDataType(StructType schema, Column column) {
        ...
        for (String part : column.getNames()) {
            ...
            StructField structField = schema.get(part);
            dataType = structField.getDataType();
        }
        return dataType;
    }
