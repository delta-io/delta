...
/**
 * A commit coordinator client that uses DynamoDB as the commit coordinator. The table schema is as
 * follows:
 ...
 * acceptingCommits: Boolean --- Whether the commit coordinator is accepting new commits. This will
 * only
 ...
 * hasAcceptedCommits: Boolean --- Whether any actual commits have been accepted by this commit
 * coordinator
 *  after `registerTable`.
...
public class DynamoDBCommitCoordinatorClient implements CommitCoordinatorClient {
    private static final Logger LOG =
            LoggerFactory.getLogger(DynamoDBCommitCoordinatorClient.class);
    ...
    private String getTableId(Map<String, String> coordinatedCommitsTableConf) {
        String tableId = coordinatedCommitsTableConf.get(TABLE_CONF_TABLE_ID_KEY);
        if (tableId == null) {
    ...
    /**
     * Fetches the entry from the commit coordinator for the given table. Only the attributes
     * defined
    ...
    protected CommitResponse commitToCoordinator(
        ...
        UpdateItemRequest request = new UpdateItemRequest()
                ...
                .addAttributeUpdatesEntry(
                        DynamoDBTableEntryConstants.TABLE_LATEST_TIMESTAMP,
                        new AttributeValueUpdate()
                                .withValue(
                                        new AttributeValue()
                                                .withN(Long.toString(inCommitTimestamp)))
                                .withAction(AttributeAction.PUT))
                .addAttributeUpdatesEntry(
        ...
        if (isCCtoFSConversion) {
            // If this table is being converted from coordinated commits to file system commits,
            // we need
        ...
        try {
            ...
            if (schemaVersion != CLIENT_VERSION) {
                throw new CommitFailedException(
                        ...
                        "The schema version of the commit coordinator does not match the current" +
                                "DynamoDBCommitCoordinatorClient version. The data schema " +
                                "version is " +
            ...
            long latestTableVersion = Long.parseLong(
                    latestEntry.getItem().get(DynamoDBTableEntryConstants.TABLE_LATEST_VERSION)
                            .getN());
            if (!skipPathCheck &&
                    !latestEntry.getItem().get("path").getS().equals(logPath.getParent()
                            .toString())) {
                throw new CommitFailedException(
                        false /* retryable */,
                        false /* conflict */,
            ...
            if (!latestEntry.getItem().get(DynamoDBTableEntryConstants.ACCEPTING_COMMITS)
                    .getBOOL()) {
                throw new CommitFailedException(
                        false /* retryable */,
                        false /* conflict */,
    ...
    public CommitResponse commit(
        ...
        try {
            ...
            Path commitPath =
                    CoordinatedCommitsDBUtils.generateUnbackfilledDeltaFilePath(
            ...
            boolean isCCtoFSConversion =
                    CoordinatedCommitsDBUtils.isCoordinatedCommitsToFSConversion(
            ...
            CommitResponse res = commitToCoordinator(
                    new Path(logPath.toString().replace("s3a", "s3")),
                    coordinatedCommitsTableConf,
                    commitVersion,
                    commitFileStatus,
                    inCommitTimestamp,
            ...
            throw new CommitFailedException(
                    false /* retryable */, false /* conflict */, e.getMessage());
        }
    }
    ...
    private GetCommitsResultInternal getCommitsImpl(
        ...
        Path unbackfilledCommitsPath = new Path(logPath, CoordinatedCommitsDBUtils.COMMIT_SUBDIR);
        for(AttributeValue attr: allStoredCommits.getL()) {
            java.util.Map<String, AttributeValue> commitMap = attr.getM();
            long commitVersion =
                    Long.parseLong(commitMap.get(DynamoDBTableEntryConstants.COMMIT_VERSION)
                            .getN());
            ...
            if (commitInRange) {
                ...
                long length =
                        Long.parseLong(commitMap.get(DynamoDBTableEntryConstants.COMMIT_FILE_LENGTH)
                                .getN());
                long modificationTime = Long.parseLong(
                        commitMap.get(
                                DynamoDBTableEntryConstants
                                        .COMMIT_FILE_MODIFICATION_TIMESTAMP).getN());
                FileStatus fileStatus = new FileStatus(
                        ...
                        filePath);
                long inCommitTimestamp =
                        Long.parseLong(commitMap.get(DynamoDBTableEntryConstants.COMMIT_TIMESTAMP)
                                .getN());
                commits.add(new Commit(commitVersion, fileStatus, inCommitTimestamp));
            }
        }
        GetCommitsResponse response = new GetCommitsResponse(commits, currentVersion);
    ...
    private void writeActionsToBackfilledFile(
        ...
        Path targetPath = CoordinatedCommitsDBUtils.getBackfilledDeltaFilePath(logPath, version);
    ...
    private void validateBackfilledFileExists(
        ...
        try {
            ...
            long version = lastKnownBackfilledVersion;
            Path lastKnownBackfilledFile =
                    CoordinatedCommitsDBUtils.getBackfilledDeltaFilePath(logPath, version);
            FileSystem fs = logPath.getFileSystem(hadoopConf);
            if (!fs.exists(lastKnownBackfilledFile)) {
                throw new IllegalArgumentException(
                        "Expected backfilled file at " +
                                lastKnownBackfilledFile + " does not exist.");
            }
        } catch (IOException e) {
            throw new UncheckedIOException(e);
        }
    ...
    /**
     * Backfills all the unbackfilled commits returned by the commit coordinator and notifies the
     * commit
    ...
    public void backfillToVersion(
        ...
        try {
            ...
            // the commit coordinator failed. The main purpose of a backfill operation is to ensure
            // that
            // UUID commit is physically copied to a standard commit file path. A failed update to
            // the commit coordinator is not critical.
            LOG.warn("Backfill succeeded but the update to the commit coordinator failed. " +
                    "This is probably" +
                    " due to a concurrent update to the commit coordinator. This is not a " +
                    "critical error and " +
    ...
    public Map<String, String> registerTable(
        ...
        // If HAS_ACCEPTED_COMMITS is false, in a getCommit request, we set the latest version to
        // -1.
    ...
    private void tryEnsureTableExists() throws IOException {
        ...
        while(retries < 20) {
            ...
            try {
                ...
                LOG.info(
                        "DynamoDB table `{}` for endpoint `{}` does not exist. " +
                                "Creating it now with provisioned throughput of {} RCUs and {} " +
                                "WCUs.",
                        coordinatedCommitsTableName,
                        endpoint, readCapacityUnits, writeCapacityUnits);
                try {
                    client.createTable(
            ...
            if (status.equals("ACTIVE")) {
                ...
                throw new RuntimeException("DynamoDBCommitCoordinatorCliet: Unable to create " +
                        "table with " +
                        "name " + coordinatedCommitsTableName + " for endpoint " + endpoint +
                        ". Ensure " +
