...
public interface FileIO
{
    ...
    /**
     * Open a {@link SeekableInputStream} at the given path which can be used to read
     * the file from any arbitrary position in the file.
     *
     * @param path Fully qualified path to the file.
     * @return {@link SeekableInputStream} instance.
     * @throws IOException for any IO error.
     */
    SeekableInputStream open(String path) throws IOException;

    /**
     * Create a new file at the given path. If the file already exists , either at
     * the time of creating the {@link OutputStream} or at the time of closing it,
     * and {@code putIfAbsent} is true, then the file will not be created and an
     * {@link java.nio.file.FileAlreadyExistsException} will be thrown. If
     *
     * @param path Fully qualified path to the file.
     * @return {@link OutputStream} instance to write to the file.
     * @throws IOException for any IO error.
     */
    OutputStream create(String path, boolean putIfAbsent) throws IOException;
