...
public class ParquetFilterUtils {
    ...
    public static Filter toParquetFilter(
            ParquetMetadata footer,
            Predicate deltaPredicate) {

        // Read the schema from the parquet file. We need to find what columns exists in the
        // Parquet file in order to remove predicates on columns that do not exist in the file.
        // Parquet filtering considers tha column that doesn't exists as a all nulls value columns
        // which may fail the filtering.
        Map<Column, ParquetField> parquetFieldMap =
                extractParquetFields(footer.getFileMetaData().getSchema());

        Optional<FilterPredicate> filterPredicate =
                convertToParquetFilter(parquetFieldMap, deltaPredicate);

        return filterPredicate.map(FilterCompat::get).orElse(FilterCompat.NOOP);
    }
    ...
    private static boolean canUseLiteral(Literal literal, PrimitiveType parquetType) {
        ...
        switch (parquetType.getPrimitiveTypeName()) {
            ...
            // TODO: Add support for decimal as int
            case INT64:
                if (!isLong(literal)) {
                    return false;
                }
            ...
            // TODO: Add support for timestamp type stored as INT64, decimal as long
            case FLOAT:
    ...
    private static Optional<FilterPredicate> convertComparatorToParquetFilter(
        ...
        if (literal.getValue() == null) {
            return visitUnsupported(
                    deltaPredicate,
                    "Literal value is null. Filtering is not supported for null values " +
                            "as the Parquet filter API is not null safe.");
        }
    ...
    private static Optional<FilterPredicate> convertAndToParquetFilter(
            Map<Column, ParquetField> parquetFieldMap,
            Predicate deltaPredicate) {
        And and = (And) deltaPredicate;
        Optional<FilterPredicate> leftFilter =
                convertToParquetFilter(parquetFieldMap, and.getLeft());
        Optional<FilterPredicate> rightFilter =
                convertToParquetFilter(parquetFieldMap, and.getRight());
    ...
    private static Optional<FilterPredicate> visitUnsupported(
            Predicate predicate,
            String message) {
        logger.debug("Unsupported predicate: {}. Reason: {}", predicate, message);
    ...
    private static boolean isInteger(Literal literal) {
        // TODO: Add support for decimal type stored as int
    ...
    private static boolean isLong(Literal literal) {
        // TODO: Add support for decimal type stored as long and timestamp stored as int64
        DataType dataType = literal.getDataType();
    ...
    private static boolean isBinary(Literal literal) {
        // TODO: Add support for decimal type stored as binary
