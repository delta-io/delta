...
class ParquetSchemaUtils {
  ...
  private static Optional<Integer> getFieldId(FieldMetadata fieldMetadata, String fieldIdKey) {
    ...
    return Optional.ofNullable(fieldMetadata.getLong(fieldIdKey))
        .map(
            fieldId -> {
              long fieldIdLong = fieldId;
              checkArgument((int) fieldIdLong == fieldIdLong, "Field id out of range", fieldIdLong);
              return (int) fieldIdLong;
            });
  }
}
