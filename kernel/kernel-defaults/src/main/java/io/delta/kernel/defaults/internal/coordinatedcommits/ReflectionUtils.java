...
public class ReflectionUtils {
    ...
    private static boolean readsCredsFromHadoopConf(Class<?> awsCredentialsProviderClass) {
        return Arrays.stream(awsCredentialsProviderClass.getConstructors())
                .anyMatch(constructor -> constructor.getParameterCount() == 1 &&
                        Arrays.equals(
                                constructor.getParameterTypes(), new Class[]{Configuration.class}));
    }

    /**
     * Creates a AWS credentials provider from the given provider classname and
     * {@link Configuration}.
     *
     ...
     *
     * @param credentialsProviderClassName Fully qualified name of the desired credentials provider
     *                                     class.
     * @param hadoopConf Hadoop configuration, used to create instance of AWS credentials
     *                                      provider, if supported.
     * @return {@link AWSCredentialsProvider} object, instantiated from the class
     * @see {credentialsProviderClassName}
     * @throws ReflectiveOperationException When AWS credentials provider constructor do not match.
     *                                      Indicates that the class has neither a constructor with
     *                                      no args
     *                                      nor a constructor with only Hadoop configuration as
     *                                      argument.
     */
