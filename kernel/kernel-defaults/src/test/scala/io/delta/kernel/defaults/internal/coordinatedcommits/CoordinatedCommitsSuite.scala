  ...
  test("getLogSegmentForVersion with commitCoordinatorClientHandler provided") {
    ...
    testWithCoordinatorCommits(config, {
      (tablePath, engine) =>
          ...
          .map(v => {
            val fileName = if (v == 2L) {
              ...
              FileNames.unbackfilledDeltaFile(
                logPath, v, Optional.of(s"uuid-${nextUuidSuffix - 1}")).toString
            }
        ...
        assert(
          logSegment.checkpointVersionOpt.isPresent && logSegment.checkpointVersionOpt.get == 1L)
        assert(logSegment.lastCommitTimestamp ==
          engine.getFileSystemClient.listFrom(
            FileNames.unbackfilledDeltaFile(
              logPath, 4L, Optional.of(s"uuid-2")).toString).next.getModificationTime)
    })
  }
