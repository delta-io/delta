...
class RowTrackingSuite extends DeltaTableWriteSuiteBase with ParquetSuiteBase {
  ...
  private def verifyBaseRowIDs(engine: Engine, tablePath: String, expectedValue: Seq[Long]) = {
    ...
    val AddFileBaseRowIDsSorted = AddFileActionsBatches.asScala
    ...
    assert(AddFileBaseRowIDsSorted === expectedValue)
  }
  ...
  private def verifyDefaultRowCommitVersion(
    ...
    val AddFileDefaultRowCommitVersionsSorted = AddFileActionsBatches.asScala
      .flatMap(_.getRows.asScala)
      .toSeq
      .map(_.getStruct(0))
      .sortBy(_.getLong(modificationTimeOrdinal))
  ...
  test("Base row IDs/default row commit versions are assigned to AddFile actions") {
    withTempDirAndEngine { (tablePath, engine) =>
      ...
      // This will commit 3 AddFile actions with 100, 200, and 400 rows respectively in one txn
  ...
  test("Fail to assign base row IDs to AddFile actions w/o stats") {
    withTempDirAndEngine { (tablePath, engine) =>
      ...
      val action = SingleAction.createAddFileSingleAction(addFile.toRow)
      ...
      val e = intercept[RuntimeException] {
        txn.commit(engine, inMemoryIterable(toCloseableIterator(Seq(action).asJava.iterator())))
      }
      assert(
  ...
  test("Integration test - Write table with Kernel then write with Spark") {
    withTempDirAndEngine((tablePath, engine) => {
      ...
      withTable(tbl) {
        ...
        // Write data using Kernel to the table
        ...
        // Append 20, 80 rows to the table using Spark
        ...
        // Verify the table state using Kernel
