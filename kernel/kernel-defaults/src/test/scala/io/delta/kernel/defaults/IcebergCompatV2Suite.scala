...
class IcebergCompatV2Suite extends DeltaTableWriteSuiteBase {
    ...
    dataType: DataType =>
      test(s"allowed data column types: $dataType on a new table") {
        withTempDirAndEngine { (tablePath, engine) =>
          ...
          createTable(engine, tablePath, schema, tableProperties = tblProps)
        }
      }
  }
    ...
    dataType: DataType =>
      test(s"allowed partition column types: $dataType on a new table") {
        withTempDirAndEngine { (tablePath, engine) =>
          ...
          val tblProps = Map(TableConfig.ICEBERG_COMPAT_V2_ENABLED.getKey -> "true")
          createTable(engine, tablePath, schema, partitionCols, tableProperties = tblProps)
        }
      }
  }
  ...
  test("enable icebergCompatV2 on a new table") {
    withTempDirAndEngine { (tablePath, engine) =>
      val tblProps = Map(TableConfig.ICEBERG_COMPAT_V2_ENABLED.getKey -> "true")
      createTable(engine, tablePath, testSchema, tableProperties = tblProps)
  ...
  Seq("id", "name").foreach { existingCMMode =>
    ...
    test(s"existing column mapping mode `$existingCMMode` is " +
      ...
      withTempDirAndEngine { (tablePath, engine) =>
        ...
        updateTable(engine, tablePath, tableProperties = newTblProps)
  ...
  test("can't be enabled on a new table with deletion vectors supported") {
    withTempDirAndEngine { (tablePath, engine) =>
      checkError[KernelException](
        ...
        createTable(engine, tablePath, schema = testSchema, tableProperties = tblProps)
      }
    }
  }
  ...
  test("column mapping mode `name` auto enabled when icebergCompatV2 is enabled on a new table") {
    withTempDirAndEngine { (tablePath, engine) =>
      val tblProps = Map(TableConfig.ICEBERG_COMPAT_V2_ENABLED.getKey -> "true")
      createTable(engine, tablePath, testSchema, tableProperties = tblProps)
      ...
      assert(protocol.supportsFeature(TableFeatures.COLUMN_MAPPING_RW_FEATURE))
      val metadata = getMetadata(engine, tablePath)
      assert(metadata.getConfiguration.get(TableConfig.COLUMN_MAPPING_MODE.getKey) === "name")
    }
  }

  test("can't enable icebergCompatV2 on a table with no column mapping enabled") {
    withTempDirAndEngine { (tablePath, engine) =>
      createTable(engine, tablePath, testSchema)

      checkError[KernelException](
        "The value 'none' for the property 'delta.columnMapping.mode' is not " +
          "compatible with Iceberg compat requirements") {
        val tblProps = Map(TableConfig.ICEBERG_COMPAT_V2_ENABLED.getKey -> "true")
        updateTable(engine, tablePath, testSchema, tableProperties = tblProps)
      }
    }
  }
