...
class DefaultJsonHandlerSuite extends AnyFunSuite with TestUtils with DefaultVectorTestUtils {
  ...
  test("read json files from middle of directory, some of which are empty") {
    val testFiles = fsClient.listFrom(getTestResourceFilePath("json-files-with-empty/5.json"))
    val actResult = jsonHandler.readJsonFiles(
      testFiles,
      new StructType()
        .add("path", StringType.STRING)
