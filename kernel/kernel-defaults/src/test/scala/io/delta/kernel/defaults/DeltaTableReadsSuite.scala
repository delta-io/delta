...
//  test("create table") {
//    val tableName =
//      "`./connectors/golden-tables/src/main/resources/golden/spark-variant-checkpoint`"
//    val query =
//      """
//    with jsonStrings as (
//      select
...
//    (0 until 2).foreach { v =>
//      spark.sql(query).where(s"id = $v").write.format("delta").mode("append").insertInto(tableName)
//    }
//  }
