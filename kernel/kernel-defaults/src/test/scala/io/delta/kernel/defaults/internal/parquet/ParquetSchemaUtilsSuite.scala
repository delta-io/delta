...
class ParquetSchemaUtilsSuite extends AnyFunSuite with TestUtils {
  ...
  test("toParquetSchema - GeometryType maps to BINARY with GEOMETRY logical annotation") {
    ...
    val field = parquetSchema.asGroupType().getType("geom").asPrimitiveType()
  ...
  test("toParquetSchema - GeographyType maps to BINARY with GEOGRAPHY logical annotation") {
    val deltaSchema = new StructType()
      .add("geog", new GeographyType("OGC:CRS84", "spherical"))

    val parquetSchema = ParquetSchemaUtils.toParquetSchema(deltaSchema)
    val field = parquetSchema.asGroupType().getType("geog").asPrimitiveType()
  ...
  test("toParquetSchema - unknown geography algorithm throws IllegalArgumentException") {
    val deltaSchema = new StructType()
      .add("geog", new GeographyType("OGC:CRS84", "unknown_algo"))

    val ex = intercept[IllegalArgumentException] {
      ParquetSchemaUtils.toParquetSchema(deltaSchema)
    }
    assert(ex.getMessage.contains("Unknown edge interpolation algorithm"))
  }
