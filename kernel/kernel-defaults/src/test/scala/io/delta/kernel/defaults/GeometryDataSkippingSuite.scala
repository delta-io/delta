...
import io.delta.kernel.expressions.{Column, Literal, StIntersectsBoxes}
import io.delta.kernel.internal.util.Utils.toCloseableIterator
import io.delta.kernel.statistics.DataFileStatistics
import io.delta.kernel.types.{DataType, GeographyType, GeometryType, StructType}
...
/**
 * End-to-end tests for data skipping with geometry and geography column types using the
 * StIntersectsBoxes predicate.
 *
 * The Kernel Parquet writer does not yet support geometry/geography columns. Tests bypass the
 * Parquet write step and inject DataFileStatistics with geometry min/max literals directly into
 * the Delta log via Transaction.generateAppendActions. This is sufficient for validating the data
 * skipping logic, which operates entirely on the Delta log stats JSON.
 */
class GeometryDataSkippingSuite extends AnyFunSuite with WriteUtils {

  // 4-quadrant layout. Files are arranged so their bounding boxes do not overlap:
  //
  //    y=10 +----------+----------+
  //         | NW file2 | NE file1 |
  //    y=7  +          +          +
  ...
  for (colType <- Seq[DataType](new GeometryType(), new GeographyType())) {
    ...
    test(s"StIntersectsBoxes data skipping on $typeName column") {
      withTempDirAndEngine { (tablePath, engine) =>
        val schema = new StructType().add("geom", colType)
        writeFilesWithGeometryStats(tablePath, engine, schema, fileExtents)
    ...
    test(s"StIntersectsBoxes: file with null $typeName stats is never skipped") {
      withTempDirAndEngine { (tablePath, engine) =>
        val schema = new StructType().add("geom", colType)
        val geomCol = new Column("geom")

        // File 0: SW quadrant with geometry stats
        // File 1: no geometry stats -> must always be included (conservative)
        val extentsOpt: Seq[Option[(Double, Double, Double, Double)]] = Seq(
          Some((0.0, 0.0, 3.0, 3.0)),
          None)

        extentsOpt.zipWithIndex.foreach { case (extentOpt, idx) =>
          val txn = if (idx == 0) getCreateTxn(engine, tablePath, schema)
          ...
          val stats = extentOpt match {
            case Some((minX, minY, maxX, maxY)) =>
              new DataFileStatistics(
                ...
                Optional.empty())
            case None =>
              // Omit geometry min/max to simulate missing geospatial stats
              new DataFileStatistics(
                10,
          ...
          val fileStatus = new DataFileStatus(filePath, 1000, 0L, Optional.of(stats))
          ...
          val actions = Transaction.generateAppendActions(
            ...
            writeContext)
          commitTransaction(txn, engine, inMemoryIterable(actions))
        }
        ...
        // NE query: SW file is provably non-overlapping, but null-stats file must be included
        assert(filesHit(8.0, 8.0, 11.0, 11.0) == 1)
        // SW query: SW file matches, null-stats file is also included
        assert(filesHit(1.0, 1.0, 4.0, 4.0) == 2)
      }
    }
  }

  test("StIntersectsBoxes combined with AND predicate on second column") {
    withTempDirAndEngine { (tablePath, engine) =>
      val schema = new StructType()
        .add("id", io.delta.kernel.types.IntegerType.INTEGER)
        .add("geom", new GeometryType())
      ...
      files.zipWithIndex.foreach { case ((idMin, idMax, gMinX, gMinY, gMaxX, gMaxY), idx) =>
        val txn = if (idx == 0) getCreateTxn(engine, tablePath, schema)
        else getUpdateTxn(engine, tablePath)
  ...
  // Writes files whose stats come entirely from manually crafted DataFileStatistics, bypassing
  // the Kernel Parquet writer (which doesn't support geometry columns today).
  private def writeFilesWithGeometryStats(
    ...
    val geomCol = new Column("geom")
    extents.zipWithIndex.foreach { case ((minX, minY, maxX, maxY), idx) =>
      val txn = if (idx == 0) getCreateTxn(engine, tablePath, schema)
      else getUpdateTxn(engine, tablePath)
