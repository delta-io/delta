...
class ColumnDefaultsSuite extends DeltaTableWriteSuiteBase {
  private val schemaWithDefault = new StructType()
    .add("a", StringType.STRING, true)
    .add("b", IntegerType.INTEGER, true, fieldMeta(-1, "127"))
  ...
  private def fieldMeta(fieldId: Int, defaultVal: String) = {
    ...
    if (fieldId != -1) {
      builder = builder
        ...
        .putString("delta.columnMapping.physicalName", s"$fieldId")
    }
    if (defaultVal != null) {
  ...
  test("allow default value in schema when the table feature is enabled") {
    ...
    withTempDirAndEngine { (tablePath, engine) =>
      ...
      assert(e.getMessage == "Kernel only supports changing Default Values on tables with "
  ...
  Seq(
    ...
    (
      ...
      new StructType()
        .add("a", StringType.STRING, true, fieldMeta(1, null))
        .add("b", ShortType.SHORT, true, fieldMeta(220, "350")))).foreach { case (name, schema) =>
    test(s"allow schema evolution with default values - $name") {
      // Create tables
      withTempDirAndEngine { (tablePath, engine) =>
        createEmptyTable(
          engine,
  ...
  Seq(
    ...
    case (dataType, value) =>
      test(s"block invalid schema changes to default values - $value") {
  ...
  // Need to make isNewTable = true to update schema
  def updateTableMetadata(
      engine: Engine,
      tablePath: String,
      schema: StructType): TransactionCommitResult = {
    appendData(
      engine,
      tablePath,
      isNewTable = true,
