...
public class IcebergCompatV2Utils {
      ...
      (inputContext) -> {
        ...
        if (!matches.isEmpty()) {
          throw new UnsupportedOperationException(
              String.format(
                  "The schema contains unsupported types: %s",
              ...
              .forEach(
                  partitonCol -> {
                    ...
                    if (!validType) {
                      throw new UnsupportedOperationException(
                          String.format(
                              "The partition column '%s' is of type ArrayType, "
  ...
  private static final IcebergCompatCheck ICEBERG_COMPAT_V2_CHECK_HAS_NO_PARTITION_EVOLUTION =
      (inputContext) -> {
        // TODO: Kernel doesn't support replace yet. When it is supported, extend
  ...
  private static final IcebergCompatCheck ICEBERG_COMPAT_V2_CHECK_HAS_NO_DELETION_VECTORS =
      (inputContext) -> {
        if (inputContext.newProtocol.supportsFeature(TableFeatures.DELETION_VECTORS_RW_FEATURE)) {
          throw new UnsupportedOperationException(
              "The table feature 'deletion_vectors' is not supported for IcebergV2 compatibility");
        }
      };
  ...
  private static final IcebergCompatCheck ICEBERG_COMPAT_V2_CHECK_HAS_SUPPORTED_TYPE_WIDENING =
      (inputContext) -> {
        if (inputContext.newProtocol.supportsFeature(TableFeatures.TYPE_WIDENING_RW_FEATURE)) {
          ...
          throw new UnsupportedOperationException(
              "The table feature 'update_vectors' is not supported for IcebergV2 compatibility");
        }
      };
