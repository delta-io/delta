...
/**
 * We don't return partial batch. If page size is returned, we terminate pagination early. We use
 * number of batches to skip as page token.
 */
  ...
  public boolean hasNext() {
    ...
    while (originalIterator.hasNext()) {
      ...
      if (!fileName.equals(lastLogFileName)) {
        ...
        if (fileName.endsWith(".parquet") && sidecarIdx != -1)
          sidecarIdx++; // keep track of sidecarIdx
      ...
      /**
       * if sideIdx isn't -1 & current file to read is checkpoint file: sidecarIdx ++ return
       * sidecarIdx;
       */
