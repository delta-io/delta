...
public class SchemaUtils {
  ...
  private static void validateNoTypeChange(StructField existingField, StructField newField) {
    ...
    if (existingField.getDataType() instanceof StructType
      ...
      Map<Integer, List<StructField>> existingStructFieldsById =
          existingStruct.fields().stream()
              .collect(Collectors.groupingBy(ColumnMapping::getColumnId));
      for (StructField newNestedField : newStruct.fields()) {
        // The existing field is guaranteed to be well-defined,
        // there should only be one field with the ID
        StructField existingNestedField =
            existingStructFieldsById.get(getColumnId(newNestedField)).get(0);

        if (existingNestedField != null) {
          validateNoTypeChange(existingNestedField, newNestedField);
        }
      }
