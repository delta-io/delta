...
public class PaginatedScanImpl implements PaginatedScan {
    ...
    public PaginatedScanImpl(StructType snapshotSchema, StructType readSchema, Protocol protocol, Metadata metadata, LogReplay logReplay,
                             Optional<Predicate> filter, Path dataPath, SnapshotReport snapshotReport,
                             String pageTokenString, long pageSize) {
        baseScan = new ScanImpl(snapshotSchema, readSchema, protocol, metadata, logReplay, filter, dataPath, snapshotReport);
    ...
    public CloseableIterator<FilteredColumnarBatch> getScanFiles(Engine engine, boolean includeStates){
        ...
        PaginationContext paginationContext = new PaginationContext(startingLogFileName, numOfAddFilesToSkip, pageToken.getSidecarIdx(), pageSize,
            isHashSetCached, tombstonesFromJson, addFilesFromJson);

        CloseableIterator<FilteredColumnarBatch> scanFileIter = baseScan.getScanFiles(engine, includeStates, paginationContext);
        PaginatedAddFilesIterator paginatedIter= new PaginatedAddFilesIterator(scanFileIter, paginationContext);
        this.pageToken = new PageToken(paginatedIter.getNextStartingLogFileName(),
            paginatedIter.getNumAddFilesToSkipForNextPage(), 0, 0); // update page token
