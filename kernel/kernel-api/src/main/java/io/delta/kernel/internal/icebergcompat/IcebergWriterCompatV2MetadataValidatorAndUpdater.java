...
/**
 * Performs the validations and updates necessary to support the table feature IcebergWriterCompatV2
 * when it is enabled by the table property "delta.enableIcebergWriterCompatV2".
 *
 ...
 * </ul>
 *
 * <p>Checks that required table features are enabled: icebergCompatWriterV2, icebergCompatV3,
 * columnMapping
 *
 ...
 */
public class IcebergWriterCompatV2MetadataValidatorAndUpdater
    extends IcebergCompatMetadataValidatorAndUpdater {

  /**
   * Validates that any change to property {@link TableConfig#ICEBERG_WRITER_COMPAT_V2_ENABLED} is
   ...
   * </ul>
   */
  public static void validateIcebergWriterCompatV2Change(
      Map<String, String> oldConfig, Map<String, String> newConfig, boolean isNewTable) {
    if (!isNewTable) {
      boolean wasEnabled = TableConfig.ICEBERG_WRITER_COMPAT_V2_ENABLED.fromMetadata(oldConfig);
      boolean isEnabled = TableConfig.ICEBERG_WRITER_COMPAT_V2_ENABLED.fromMetadata(newConfig);
      if (!wasEnabled && isEnabled) {
        throw DeltaErrors.enablingIcebergWriterCompatV2OnExistingTable(
            TableConfig.ICEBERG_WRITER_COMPAT_V2_ENABLED.getKey());
      }
      if (wasEnabled && !isEnabled) {
        throw DeltaErrors.disablingIcebergWriterCompatV2OnExistingTable(
            TableConfig.ICEBERG_WRITER_COMPAT_V2_ENABLED.getKey());
      }
    }
  }
  ...
  /**
   * Validate and update the given Iceberg Writer Compat V2 metadata.
   *
   ...
   * @throws UnsupportedOperationException if the metadata is not compatible with Iceberg Writer V2
   *     requirements
   */
  public static Optional<Metadata> validateAndUpdateIcebergWriterCompatV2Metadata(
  ...
  /// //////////////////////////////////////////////////////////////////////////////
  /// Define the compatibility and update checks for icebergWriterCompatV2       ///
  /// //////////////////////////////////////////////////////////////////////////////

  private static final IcebergWriterCompatV2MetadataValidatorAndUpdater INSTANCE =
      new IcebergWriterCompatV2MetadataValidatorAndUpdater();
  ...
  private static final IcebergCompatRequiredTablePropertyEnforcer CM_ID_MODE_ENABLED =
      new IcebergCompatRequiredTablePropertyEnforcer<>(
          ...
          // name is correctly set as part of icebergWriterCompatV2 checks
          (inputContext) ->
  ...
  private static final IcebergCompatCheck UNSUPPORTED_FEATURES_CHECK =
      (inputContext) -> {
        ...
        //   for blocking enablement of icebergWriterCompatV2 versus enablement of the incompatible
        //   feature
   ...
   * {@link ShortType} are unsupported for IcebergWriterCompatV2 tables.
   */
  ...
  String compatFeatureName() {
    return "icebergWriterCompatV2";
  }

  @Override
  TableConfig<Boolean> requiredDeltaTableProperty() {
    return TableConfig.ICEBERG_WRITER_COMPAT_V2_ENABLED;
  }
  ...
  List<TableFeature> requiredDependencyTableFeatures() {
    return Stream.of(
            ICEBERG_WRITER_COMPAT_V2, ICEBERG_COMPAT_V3_W_FEATURE, COLUMN_MAPPING_RW_FEATURE)
        .collect(toList());
  }
