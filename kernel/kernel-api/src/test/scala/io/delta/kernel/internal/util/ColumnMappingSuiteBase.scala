...
trait ColumnMappingSuiteBase extends VectorTestUtils {
  ...
  /**
   * Verify the schema returned by [[testSchema()]] has correct column mapping (including nested)
   * info assigned
   */
  def verifyTestSchemaHasValidColumnMappingInfo(
      metadata: Metadata,
      isNewTable: Boolean = true): Unit = {
    ...
    assertColumnMapping(innerStruct.get("f"), 5L, if (isNewTable) "UUID" else "f")
    ...
    assertColumnMapping(structInArray.get("g"), 6L, if (isNewTable) "UUID" else "g")
    assertColumnMapping(structInArray.get("h"), 7L, if (isNewTable) "UUID" else "h")
    assertColumnMapping(metadata.getSchema.get("c"), 8L, if (isNewTable) "UUID" else "c")
