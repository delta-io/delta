  ...
  test("ScanReport serializer") {
    ...
    val expectedJson =
      s"""
         ...
         |"exception":"$exception",
         |"tableVersion":1,
         |"tableSchema":"struct(StructField(name=part,type=integer,nullable=true,metadata={},typeChanges=[]), StructField(name=id,type=integer,nullable=true,metadata={},typeChanges=[]))",
         |"snapshotReportUUID":"$snapshotReportUUID",
         |"filter":"(column(`part`) > 1)",
         |"readSchema":"struct(StructField(name=id,type=integer,nullable=true,metadata={},typeChanges=[]))",
