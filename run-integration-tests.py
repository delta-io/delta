...
def run_python_integration_tests(root_dir, version, test_name, extra_maven_repo, use_local):
    ...
    package = f"io.delta:delta-core_2.12:{version}"

    repo = extra_maven_repo or ""
    ...
    for test_file in test_files:
        ...
        try:
            cmd = ["spark-submit",
                   f"--driver-class-path={extra_class_path}",  # for less verbose logging
                   "--packages", package,
...
def run_dynamodb_logstore_integration_tests(root_dir, version, test_name, extra_maven_repo,
    ...
    python_root_dir = path.join(root_dir, "python")
    extra_class_path = path.join(python_root_dir, path.join("delta", "testing"))
    packages = f"io.delta:delta-core_2.12:{version}"
