...
public class CatalogManagedTableIntTest extends IntTestBase {
  ...
  void testLoadTable() throws Exception {
    try (CatalogManagedTable table =
      ...
      assertTrue(table.getSchema().equivalent(SCHEMA_WITH_ALL_TYPES));
    }
  }

  @IntTest
  void testWriteTable() throws Exception {
    StructType schema = SCHEMA_WITH_ALL_TYPES;
    ...
    for (int i = 0; i < 100; i++) {
      List<List<?>> values = dummyData(schema, 10);
      ColumnVector[] cvs =
          IntStream.range(0, schema.fields().size())
              .mapToObj(
                  index ->
                      new DataColumnVectorView(
                          values, index, schema.fields().get(index).getDataType()))
              .toArray(ColumnVector[]::new);

      DefaultColumnarBatch columnarBatchData =
          new DefaultColumnarBatch(values.size(), table.getSchema(), cvs);
