...
/**
 * Flink {@link CatalogTable} backed by a Unity Catalog {@link TableInfo}. Converts UC column
 * metadata (type JSON) to Flink's {@link Schema} and injects Delta connector options when the
 * table's data source format is DELTA.
 */
public class FlinkUnityCatalogTable implements CatalogTable {
  ...
  static final Pattern decimalPattern = Pattern.compile("decimal\\((\\d+),(\\d+)\\)");
