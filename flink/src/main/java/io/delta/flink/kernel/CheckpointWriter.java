 ...
 * </ul>
 *
 * If the writer detects that the number of sidecar files exceeds a threshold, it will merge them
 * into a new single sidecar files to reduce the total number of files.
 *
...
public class CheckpointWriter {
   ...
   */
  public static final String TAG_DELTASINK = "io.delta.flink.sink";
  ...
  public CheckpointWriter(Engine engine, Snapshot snapshot, int sidecarMergeThreshold) {
    ...
    lastCheckpointByMe =
        Boolean.parseBoolean(lastCheckpointMeta.tags.getOrDefault(TAG_DELTASINK, "false"));
    lastSidecarCount = 0;
    try {
      lastSidecarCount =
   ...
   * #TAG_DELTASINK}), the writer behaves as if no previous checkpoint exists.
   *
  ...
  public void write() throws IOException {
    ...
    // Only use the information from _last_checkpoint when
    ...
    try (CloseableIterator<FilteredColumnarBatch> actions =
      ...
      // Incremental generation. Read existing sidecars from the previous checkpoint (only if it
   ...
   *
   * @param nameToCounters a map of 1. name of the column that must be non-null for a row to be
   *     retained 2. counter incremented for each retained row; may be null
