...
class DeltaTableBuilderSuite extends DeltaQueryTest with RemoteSparkSession {
  ...
  test("create table with identity columns") {
    withTempPath { dir =>
      ...
      verifyTestTableMetadata(
        table = s"delta.`$path`",
        schemaString = "id1 long, id2 long, id3 long, id4 long",
        identityColumns = Map(
          "id1" -> "GENERATED ALWAYS AS IDENTITY (START WITH 1 INCREMENT BY 1)",
          "id2" -> "GENERATED BY DEFAULT AS IDENTITY (START WITH 1 INCREMENT BY 1)",
          "id3" -> "GENERATED ALWAYS AS IDENTITY (START WITH 100 INCREMENT BY 10)",
          "id4" -> "GENERATED BY DEFAULT AS IDENTITY (START WITH 100 INCREMENT BY 10)"
        )
      )
    }
  }
