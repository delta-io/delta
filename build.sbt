  ...
  val kernelDepSettings: Seq[Setting[_]] = kernelVersionOpt match {
    ...
    case Some(v) => Seq(
      libraryDependencies ++= Seq(
        ...
        // Consume them via the published tests-classifier jar (made available by
        // `Test / publishArtifact := true` on kernelUnityCatalog).
...
lazy val kernelUnityCatalog = (project in file("kernel/unitycatalog"))
  ...
  .settings (
    ...
    // UCCatalogManagedTestUtils) via a published artifact. Mirrors the pattern
    // kernelApi already uses.
    Test / publishArtifact := true,
