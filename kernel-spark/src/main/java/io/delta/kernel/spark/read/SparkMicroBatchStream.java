  ...
  private KernelSourceOffset getStartingOffsetFromSpecificVersion(
    ...
    try {
      if (isInitialSnapshot) {
        ...
        // Use CommitRange to scan for file changes from the starting version
        ...
        // Look for add file actions in the commit range
        ...
        try (CloseableIterator<ColumnarBatch> actionIter =
          ...
          while (actionIter.hasNext()) {
            ...
            if (batch.getSize() > 0) {
              // Get the version from this batch
              ColumnVector versionVector = batch.getColumnVector(0); // version is first column
              for (int i = 0; i < batch.getSize(); i++) {
                long version = versionVector.getLong(i);
                lastVersionWithChanges = Math.max(lastVersionWithChanges, version);
                lastIndex = i; // Use the row index as the file index
              }
            }
          }

          if (lastVersionWithChanges == fromVersion && lastIndex == KernelSourceOffset.BASE_INDEX) {
            // No file changes found
            return null;
          }

          return KernelSourceOffset.apply(
              tableId, lastVersionWithChanges, lastIndex, false // Not an initial snapshot
              );
        }
      }
    } catch (Exception e) {
