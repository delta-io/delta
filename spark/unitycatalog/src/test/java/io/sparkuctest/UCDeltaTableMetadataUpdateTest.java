...
public class UCDeltaTableMetadataUpdateTest extends UCDeltaTableIntegrationBaseTest {
  ...
  /**
   * RESTORE TABLE to a version whose clustering differs from the current version must succeed.
   *
   * <p>The table first changes clustering through a normal ALTER TABLE commit so UC and the Delta
   * log stay in sync, then RESTORE moves the table back to the earlier clustering metadata.
   */
  @Test
  public void testRestoreTableWithClusteringChangeSucceeds() throws Exception {
    ...
    try {
      ...
      sql("RESTORE TABLE %s TO VERSION AS OF %d", tableName, versionBeforeClusteringChange);
