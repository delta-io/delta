...
import io.sparkuctest.extensions.UCExtension;
 ...
 * <p>
 * This class provides a pluggable SQL execution framework via the SQLExecutor interface, allowing
 * tests to be written once and executed via different execution engines (e.g., Spark SQL, JDBC,
 * REST API, etc.).
 * <p>
 * Subclasses must provide an executor by implementing the getSqlExecutor method.
 */
@TestInstance(TestInstance.Lifecycle.PER_CLASS)
public abstract class UCDeltaTableIntegrationBaseTest {

  @RegisterExtension
  public static final UCExtension UC_EXTENSION = UCExtension.initialize();
   ...
   */
  protected void withNewTable(String tableName, String tableSchema, TableType tableType,
      TestCode testCode) throws Exception {
    String fullTableName = String.format("%s.%s.%s",
        UC_EXTENSION.catalogName(),
        UC_EXTENSION.schemaName(),
        tableName);
    if (tableType == TableType.EXTERNAL) {
      // External table requires a location
      UC_EXTENSION.withTempDir((Path dir) -> {
        Path tablePath = new Path(dir, fullTableName);
        sql("CREATE TABLE %s (%s) USING DELTA LOCATION '%s'",
            fullTableName, tableSchema, tablePath);
   ...
   * <p>
   * This abstraction allows tests to be independent of the execution engine, making it easy to test
   * the same logic via different interfaces (Spark SQL, JDBC, etc.).
   */
   ...
   * <p>
   * This executor runs all SQL queries through Spark SQL and converts results to string lists for
   * easy comparison.
   */
