...
import static io.delta.storage.commit.uccommitcoordinator.UCCommitCoordinatorClient.UC_TABLE_ID_KEY;
import static io.delta.storage.commit.uccommitcoordinator.UCCommitCoordinatorClient.UC_TABLE_ID_KEY_OLD;
...
public class UCTableCreationTest extends UCDeltaTableIntegrationBaseTest {
  ...
  private static final String SUPPORTED = TableFeatureProtocolUtils.FEATURE_PROP_SUPPORTED();
  ...
  private static final List<TableFeature> EXPECTED_MANAGED_TABLE_FEATURES =
      List.of(
          AppendOnlyTableFeature$.MODULE$,
          CatalogOwnedTableFeature$.MODULE$,
          InCommitTimestampTableFeature$.MODULE$,
          InvariantsTableFeature$.MODULE$,
          VacuumProtocolCheckTableFeature$.MODULE$);
  private static final Map<String, String> EXPECTED_MANAGED_TABLE_FEATURES_PROPERTIES =
      EXPECTED_MANAGED_TABLE_FEATURES.stream()
          .map(TableFeature::propertyKey)
        ...
        for (boolean withCluster : List.of(true, false)) {
          ...
          for (boolean withAsSelect : List.of(true, false)) {
            for (boolean replaceTable : List.of(true, false)) {
              String tableName = "test_delta_table_" + counter;
              counter++;
              ...
              if (tableType == TableType.EXTERNAL) {
                assertThat(tableInfo.getStorageLocation())
                    .isEqualTo(options.getExternalTableLocation());
              }
              ...
              if (tableType == TableType.MANAGED) {
                ...
                expectedProperties.forEach(
                    (key, value) ->
                        assertThat(tablePropertiesFromServer).containsEntry(key, value));
