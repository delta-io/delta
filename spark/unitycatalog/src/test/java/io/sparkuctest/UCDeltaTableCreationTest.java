...
public class UCDeltaTableCreationTest extends UCDeltaTableIntegrationBaseTest {
  ...
  private void assertUCTableInfo(
    ...
    if (tableType == TableType.MANAGED) {
      ...
      // CLUSTER BY enables the feature; some catalog paths also store clusteringColumns.
      ImmutableMap.Builder<String, String> expectedClusteringPropertiesBuilder =
          ImmutableMap.builder();
      if (withCluster) {
        if (expectClusteringColumnsProperty) {
          expectedClusteringPropertiesBuilder.put(
              "clusteringColumns", "[[\"" + clusterColumn.get() + "\"]]");
        }
        expectedClusteringPropertiesBuilder.put("delta.feature.clustering", SUPPORTED);
      }
      final Map<String, String> expectedClusteringProperties =
          expectedClusteringPropertiesBuilder.build();
