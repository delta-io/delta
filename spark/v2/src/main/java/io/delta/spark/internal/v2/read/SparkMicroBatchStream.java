...
public class SparkMicroBatchStream
  ...
  private List<IndexedFile> collectAndBuildCDCIndexedFiles(
    ...
    // Single ordered list for inferred CDC actions (AddFile | RemoveFile) to preserve
    // delta-log action ordering. DSv1 assigns IndexedFile.index via zipWithIndex on the
    // original action sequence; splitting into separate lists would reorder them.
    List<Object> inferredCdcActions = new ArrayList<>();
    ...
    try (CloseableIterator<ColumnarBatch> actionsIter = commit.getActions()) {
      while (actionsIter.hasNext()) {
        ...
        for (int rowId = 0; rowId < batch.getSize(); rowId++) {
          ...
          Optional<RemoveFile> removeOpt = StreamingHelper.getDataChangeRemove(batch, rowId);
          if (removeOpt.isPresent()) {
            inferredCdcActions.add(removeOpt.get());
            continue;
          }
          ...
          Optional<AddFile> addOpt = StreamingHelper.getAddFileWithDataChange(batch, rowId);
          if (addOpt.isPresent()) {
            inferredCdcActions.add(addOpt.get());
          }
        }
      }
    }
    ...
    if (!cdcFiles.isEmpty()) {
      ...
      // Infer CDC from AddFile (insert) and RemoveFile (delete), preserving delta-log order
      for (Object action : inferredCdcActions) {
        if (action instanceof AddFile) {
          result.add(
              IndexedFile.cdc(
                  version, fileIndex++, CDCDataFile.fromAddFile((AddFile) action, timestamp)));
        } else {
          result.add(
              IndexedFile.cdc(
                  version,
                  fileIndex++,
                  CDCDataFile.fromRemoveFile((RemoveFile) action, timestamp)));
        }
      }
    }
