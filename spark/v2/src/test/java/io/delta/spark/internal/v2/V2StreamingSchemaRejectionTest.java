...
public class V2StreamingSchemaRejectionTest extends V2TestBase {
   ...
   */
  @Test
  public void testCase1_streamingShouldNotDropNullColumns_v1Flag_false(@TempDir File sourceDir) {
    ...
    // Capture whatever DSv2 throws when loading a VOID-bearing table.
   ...
   */
  @Test
  public void testCase2_streamingShouldDropNullColumns_v1Flag_true(@TempDir File sourceDir) {
    String sourcePath = sourceDir.getAbsolutePath();
  ...
  public void testCase3_noSchemaShouldThrow(@TempDir File inputDir) {
    ...
    String dsv2TableRef = str("dsv2.delta.`%s`", inputDir.getAbsolutePath());
  ...
  public void testCase4a_disallowUserSchema_mismatched(@TempDir File inputDir) {
    ...
    Dataset<Row> df;
    try {
      df = spark.readStream().schema(userSchema).table(dsv2TableRef);
    } catch (Throwable t) {
      caught.set(t);
      df = null;
    }

    // BUG: DSv2 silently accepts the mismatched schema. If this changes, re-classify.
    assertNull(
        caught.get(),
        "DSv2 currently silently accepts a mismatched user-specified schema. If this assertion "
            + "fails (DSv2 now throws), the parity gap with DSv1's "
            + "DELTA_READ_SOURCE_SCHEMA_CONFLICT has been closed — re-classify Case 4a as PASS.");
    assertNotNull(df);
  ...
  public void testCase4b_disallowUserSchema_matching(@TempDir File inputDir) {
    ...
    Dataset<Row> df;
    try {
      df = spark.readStream().schema(matchingSchema).table(dsv2TableRef);
    } catch (Throwable t) {
      caught.set(t);
      df = null;
    }

    // BUG: DSv2 silently accepts a user-specified schema (even a matching one). DSv1 always
    // rejects with "does not support user-specified schema".
    assertNull(
        caught.get(),
        "DSv2 currently silently accepts a (matching) user-specified schema. If this assertion "
            + "fails (DSv2 now throws), the parity gap with DSv1's 'does not support "
            + "user-specified schema' contract has been closed — re-classify Case 4b as PASS.");
    assertNotNull(df);
    assertTrue(df.isStreaming());
  }
