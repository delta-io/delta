...
/**
 * End-to-end DSv2 streaming repro for the post-PR-#6578 silent-corruption bug in {@link
...
public class V2StreamingDeletionVectorVariantTest extends V2TestBase {
  ...
  public void testStreamingReadWithDeletionVectorAndVariant(@TempDir File deltaTablePath)
    ...
    // 4. Stream via dsv2 catalog with Trigger.AvailableNow.
    ...
    // 5. Assert per-row identity: variant_get(v,'$.row','int') == id for every output row.
    //    Materialize via memory sink, then project (id, variant_get) and compare.
    String queryName = "dv_variant_repro";
        ...
        assertEquals(
            ...
            () ->
                "Silent corruption: row id="
                    ...
                    + " (expected "
                    + id
                    + "). ColumnVectorWithFilter.getChild() likely dropped row-id mapping for"
                    + " VARIANT.");
      }
    } finally {
  ...
  public void testStreamingReadWithVariantControl(@TempDir File deltaTablePath) throws Exception {
    ...
    String queryName = "dv_variant_control";
    StreamingQuery query = null;
    try {
      query =
          streamingDF
