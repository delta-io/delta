...
object IctTestUtils {
  ...
  /** Reads the ICT recorded in the commit JSON for the given version. */
  def getIct(deltaLog: DeltaLog, version: Long): Long = {
    val deltaFile = DeltaCommitFileProvider(deltaLog.update()).deltaFile(version)
    val ci = org.apache.spark.sql.delta.DeltaHistoryManager.getCommitInfoOpt(
      deltaLog.store, deltaFile, deltaLog.newDeltaHadoopConf())
    require(ci.isDefined, s"CommitInfo missing for version $version")
    require(ci.get.inCommitTimestamp.isDefined, s"ICT missing for version $version")
    ci.get.inCommitTimestamp.get
  }

  /** Returns the file modification time of the delta file for the given version. */
  def getMtime(deltaLog: DeltaLog, version: Long): Long = {
    val deltaFilePath = DeltaCommitFileProvider(deltaLog.update()).deltaFile(version)
    new File(deltaFilePath.toUri).lastModified()
  }
}
