      ...
      case (_, true) => // Enabling now or already-enabled
        ...
        requiredTableProperties.foreach {
          case RequiredDeltaTableProperty(
              deltaConfig, validator, autoSetValue, shouldAutoSetOnlyOnCreateOrReorg) =>
            ...
            if (!newestValueOkay) {
              if (!newestValueExplicitlySet &&
                  (isCreatingOrReorgTable || !shouldAutoSetOnlyOnCreateOrReorg)) {
                ...
                // If shouldAutoSetOnlyOnCreateOrReorg = false, it auto sets in all cases
 ...
 * @param shouldAutoSetOnlyOnCreateOrReorg this can be false only when the feature
...
case class RequiredDeltaTableProperty[T](
    ...
    validator: T => Boolean,
    autoSetValue: String,
    shouldAutoSetOnlyOnCreateOrReorg: Boolean = true) {
  /**
