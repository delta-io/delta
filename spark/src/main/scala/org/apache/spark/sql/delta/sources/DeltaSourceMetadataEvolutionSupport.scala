...
object DeltaSourceMetadataEvolutionSupport {
  ...
  /**
   * Delegates to [[SchemaEvolutionUtils.validateIfSchemaChangeCanBeUnblocked]].
  ...
  protected[sources] def validateIfSchemaChangeCanBeUnblocked(
    ...
    SchemaEvolutionUtils.validateIfSchemaChangeCanBeUnblocked(
      spark, parameters, metadataPath, currentSchema, previousSchema)
  }

  def getCheckpointHash(path: String): Int = SchemaEvolutionUtils.getCheckpointHash(path)

  def allowTypeWidening(spark: SparkSession): Boolean =
    SchemaEvolutionUtils.allowTypeWidening(spark)

  def bypassTypeChangeCheck(spark: SparkSession): Boolean =
    SchemaEvolutionUtils.bypassTypeChangeCheck(spark)
}
