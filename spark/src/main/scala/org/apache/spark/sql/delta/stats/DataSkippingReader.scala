...
trait DataSkippingReaderBase
      ...
      protected val dataSkippingType: DeltaDataSkippingType)
  {
    ...
    private[stats] def constructDataFilters(dataFilter: Expression):
      ...
      // For these null-intolerant expressions, any null input resolves into a null output. Push
      // down the IsNull operator to all children and return a DataSkippingPredicate that's the Or
      // of all child expressions.
      case IsNull(e: GreaterThan) => constructIsNullFilterForNullIntolerant(e)
      case IsNull(e: GreaterThanOrEqual) => constructIsNullFilterForNullIntolerant(e)
      case IsNull(e: LessThan) => constructIsNullFilterForNullIntolerant(e)
      case IsNull(e: LessThanOrEqual) => constructIsNullFilterForNullIntolerant(e)
      case IsNull(e: EqualTo) => constructIsNullFilterForNullIntolerant(e)
      case IsNull(e: Not) => constructIsNullFilterForNullIntolerant(e)
      case IsNull(e: StartsWith) => constructIsNullFilterForNullIntolerant(e)
      // And and Or necessitate custom pushdown logic for IsNull, as both expressions are not
      // considered null intolerant.
      case IsNull(And(left, right)) =>
