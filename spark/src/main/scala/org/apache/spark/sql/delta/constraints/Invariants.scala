...
object Invariants {
  ...
  def getFromSchema(schema: StructType, spark: SparkSession): Seq[Constraint] = {
    val nullConstraints = if (spark.conf.get(DeltaSQLConf.IMPLICIT_NOT_NULL_CONSTRAINT_ENABLED)) {
      SchemaUtils.filterRecursively(schema, checkComplexTypes = false)(!_.nullable).map {
        case (parents, field) => Constraints.NotNull(parents :+ field.name)
      }
    } else {
      Nil
    }
