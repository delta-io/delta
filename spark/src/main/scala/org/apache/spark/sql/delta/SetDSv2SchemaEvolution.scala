 ...
 *
 * TODO(LC-13880): Long-term: deprecate enabling schema evolution via writer option / spark conf
 * in favor of Spark's `.withSchemaEvolution()` and `WITH SCHEMA EVOLUTION` SQL syntax.
 */
class SetDSv2SchemaEvolution(session: SparkSession) extends Rule[LogicalPlan] {
