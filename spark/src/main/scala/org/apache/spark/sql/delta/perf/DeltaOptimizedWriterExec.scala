...
case class DeltaOptimizedWriterExec(
  ...
  /**
   * Reducers that computeBins() split across more than one bin. Only these need a bounded
   * map-index range at read time; every other reducer is wholly contained in its bin and can
   * be read as a full partition. This matters because a bounded range forces a remote shuffle
   * service to sort the partition file by map id before serving it (Celeborn: FetchHandler ->
   * PartitionFilesSorter), which rewrites the whole file and is bounded by
   * celeborn.worker.sortPartition.timeout.
   */
  private var splitReducerIds: Set[Int] = Set.empty
  ...
  private def computeBins(): Array[List[(BlockManagerId, ArrayBuffer[(BlockId, Long, Int)])]] = {
    ...
    val bins = if (useShuffleManager) {
      ...
      val splitIds = mutable.Set[Int]()
      val largeBins = largeReducers.flatMap { case (reducerId, blocks, _) =>
        ...
        if (chunks.length > 1) splitIds += reducerId
      ...
      splitReducerIds = splitIds.toSet
...
private class OptimizedWriterShuffleReader(
    ...
    useShuffleManager: Boolean,
    splitReducerIds: Set[Int]) extends ShuffleReader[Int, InternalRow] {
  ...
  override def read(): Iterator[Product2[Int, InternalRow]] = {
    ...
    if (useShuffleManager) {
      ...
      val rangesByReducer = blocks.flatMap { case (_, blockList) =>
        ...
        if (splitReducerIds.contains(reducerId)) {
          val mapIndexes = pairs.map(_._2)
          (reducerId, mapIndexes.min, mapIndexes.max + 1)
        } else {
          // This bin holds the reducer in full, so ask for the whole partition. A bounded
          // range would make a remote shuffle service sort the partition file by map id
          // first; Int.MaxValue is the documented "not a range read" signal and skips it.
          (reducerId, 0, Int.MaxValue)
        }
      }
