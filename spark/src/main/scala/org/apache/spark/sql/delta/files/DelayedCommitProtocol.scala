...
class DelayedCommitProtocol(
  ...
  protected def parsePartitions(
    ...
    val partitionColTypes: Map[String, DataType] =
      taskContext.asInstanceOf[PartitionedTaskAttemptContextImpl]
          .partitionAttr.groupBy(attr => attr.name)
          .filter(tup => tup._2.head.dataType == TimestampType)
          .map(tup => (tup._1, tup._2.head.dataType))
    ...
    val parsedPartition =
      PartitionUtils
        .parsePartition(
          ...
          typeInference = false,
          Set.empty,
          userSpecifiedDataTypes = partitionColTypes,
