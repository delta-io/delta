...
object ColumnWithDefaultExprUtils extends DeltaLogging {
  ...
  def addDefaultExprsOrReturnConstraints(
    ...
    // Handle generated columns and their constraints
    var selectExprs: Seq[Column] = schema.flatMap { f =>
      GeneratedColumn.getGenerationExpression(f) match {
        case Some(expression) if GeneratedColumn.satisfyGeneratedColumnProtocol(protocol) =>
          if (topLevelOutputNames.contains(f.name)) {
            ...
            constraints += Constraints.Check(s"Generated Column",
              EqualNullSafe(ColumnExpressionUtils.toExpression(data.sparkSession,
                column), expression))
            track += f.name
            Some(expr(expression.sql).alias(f.name))
          } else {
            Some(expr(expression.sql).alias(f.name))
          }
        case _ =>
          if (!topLevelOutputNames.contains(f.name) && f.nullable && nullAsDefault) {
            Some(functions.lit(null).cast(f.dataType).alias(f.name))
          } else if (topLevelOutputNames.contains(f.name)) {
    ...
    val cdcSelectExprs = CDCReader.CDC_COLUMNS_IN_DATA.flatMap { cdcColumnName =>
      topLevelOutputNames.get(cdcColumnName).map { cdcField =>
        if (!metadataOutputNames.contains(cdcColumnName)) {
          SchemaUtils.fieldToColumn(cdcField).alias(cdcField.name)
        } else {
          SchemaUtils.fieldToColumn(cdcField)
        }
      }
    }
