...
object UCCommitCoordinatorBuilder
  ...
  private[delta] def getCatalogConfigMap(spark: SparkSession): Map[String, UCCatalogConfig] = {
    getCatalogConfigs(spark).map {
      case (name, ucClientParams) => name -> UCCatalogConfig(name, ucClientParams)
    }.toMap
  }
 ...
 */
case class UCCatalogConfig(catalogName: String, ucClientParams: UCClientParams)
