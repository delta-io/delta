...
object DeltaRestSchemaConverter {
  ...
  private def buildSparkMetadata(metadata: JMap[_, _]): Metadata = {
    ...
    metadata.asScala.foreach {
      ...
      case (key, _) =>
        throw new IllegalArgumentException(s"Unsupported Delta REST metadata key: $key")
    }
    builder.build()
  }
    ...
    value match {
      ...
      case unsupported =>
        throw new IllegalArgumentException(
          s"Unsupported Delta REST metadata value for key $key: $unsupported")
    }
  }
  ...
  private def putMetadataArray(
    ...
    if (values.forall(_.isInstanceOf[String])) {
      ...
      throw new IllegalArgumentException(
        s"Unsupported Delta REST metadata array for key $key: $values")
    }
  }
