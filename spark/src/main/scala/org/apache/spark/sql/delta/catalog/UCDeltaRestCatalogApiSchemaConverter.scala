...
import io.unitycatalog.client.delta.model.{
  ...
  DecimalType => DeltaDecimalType,
  DeltaType,
  MapType => DeltaMapType,
  PrimitiveType,
...
private[catalog] object UCDeltaRestCatalogApiSchemaConverter {
  ...
  def toSparkType(schema: DeltaStructType): StructType = {
    StructType(schema.getFields.asScala.map(toSparkField).toSeq)
  }

  private def toSparkField(field: DeltaStructField): StructField = {
    StructField(
      name = field.getName,
