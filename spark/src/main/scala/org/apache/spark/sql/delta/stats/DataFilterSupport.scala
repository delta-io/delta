...
private[delta] object DefaultDataFilterSupport extends DataFilterSupport {
  ...
  override def buildStatsSchema(
      ...
      getFieldName: StructField => String = _.name,
      tightBoundsSupported: Boolean = false): StructType = {
    val minMaxStatsSchemaOpt = getMinMaxStatsSchema(tableSchema, getFieldName)
    val nullCountSchemaOpt = getNullCountSchema(tableSchema, getFieldName)
    val tightBoundsFieldOpt = Option.when(tightBoundsSupported)(TIGHT_BOUNDS -> BooleanType)
  ...
  private def getMinMaxStatsSchema(
      schema: StructType,
    ...
    val fields = schema.fields.flatMap {
      case f @ StructField(_, dataType: StructType, _, _) =>
        getMinMaxStatsSchema(dataType, getFieldName).map { newDataType =>
          StructField(getFieldName(f), newDataType)
        }
  ...
  private def getNullCountSchema(
      schema: StructType,
      getFieldName: StructField => String): Option[StructType] = {
    val fields = schema.fields.flatMap {
      case f @ StructField(_, dataType: StructType, _, _) =>
        getNullCountSchema(dataType, getFieldName).map { newDataType =>
          StructField(getFieldName(f), newDataType)
        }
      case f: StructField =>
