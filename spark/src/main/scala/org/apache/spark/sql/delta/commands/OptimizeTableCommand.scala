...
class OptimizeExecutor(
  ...
  private def pruneCandidateFileList(
    ...
    def shouldRewriteBecauseOfMissingIcebergCompatTag(file: AddFile): Boolean = {
      val icebergCompatVersion =
        file.tags.getOrElse(AddFile.Tags.ICEBERG_COMPAT_VERSION.name, "0")
      optimizeContext.icebergCompatVersion.isDefined &&
        icebergCompatVersion != optimizeContext.icebergCompatVersion.toString
    }
    ...
    files.filter(
      addFile => addFile.size < minFileSize || shouldCompactBecauseOfDeletedRows(addFile) ||
        shouldRewriteBecauseOfMissingIcebergCompatTag(addFile))
  }
