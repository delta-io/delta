...
/**
 * Shared base for the GeoSpatial table feature tests. Verifies:
 ...
 *
 * These tests are identical for Spark 4.1 and 4.2, so they live in the shared
 * `scala-shims/spark-4.1-4.2` test dir. Version-specific tests live in the per-version
 * `DeltaGeoSuite` subclasses (Spark's `GeometryType` / `GeographyType` catalyst types were
 * added in Spark 4.1 via SPARK-53760, so the GeoSpatial feature is a no-op on Spark 4.0 and
 * has no suite there; the Spark 4.2 subclass additionally covers the end-to-end DML/CDF/MERGE
 * paths that OSS Spark 4.1's Parquet writer cannot yet encode).
 */
trait DeltaGeoSuiteBase extends QueryTest
  ...
  protected val DefaultSrid = 4326

  protected def metadataWithSchema(schema: StructType): Metadata = {
