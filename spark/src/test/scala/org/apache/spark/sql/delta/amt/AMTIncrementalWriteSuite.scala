...
class AMTIncrementalWriteSuite extends AMTCheckpointTestBase {
  ...
  private def createTables(
    ...
    val locationClause = amtTableLocation.map(l => s"LOCATION '$l'").getOrElse("")
    sql(
      s"""CREATE TABLE $amtDeltaTable (id INT) USING DELTA
         |$locationClause
         |TBLPROPERTIES (
  ...
  /**
   * Emits an AMT checkpoint on the AMT table via the real commit path and returns the incremental
   * write's shape metrics. `incremental = false` forces a full rewrite; `true` an incremental one.
   * The metrics are read back out of the logged [[CommitStats]].
   */
  private def checkpointAMTTable(
      amtDeltaLog: DeltaLog, incremental: Boolean): Option[IncrementalAMTWriteMetrics] = {
    val trigger =
      if (incremental) AMTTriggerMode.CheckpointIntervalIncremental
      else AMTTriggerMode.CheckpointIntervalFull
    val attemptVersion = amtDeltaLog.update().version + 1
    metricsOf(attemptVersion) {
      amtDeltaLog.startTransaction().commit(
        Seq.empty, DeltaOperations.OptimizeCheckpoint(incremental, trigger.name))
    }
  }

  /**
   * Runs `commit` and returns the [[IncrementalAMTWriteMetrics]] logged for the commit at
   * `commitVersion`, or None if that commit wrote a full (non-incremental) AMT.
   */
  ...
  /** Emits a full AMT checkpoint on the AMT table (setup step), then validates the tree. */
  private def fullCheckpoint(amtDeltaLog: DeltaLog): Unit = {
    checkpointAMTTable(amtDeltaLog, incremental = false)
  ...
  private def createIncrementalAMTAndValidate(
    ...
    val actualIncrementalAMTWriteMetrics = checkpointAMTTable(amtDeltaLog, incremental = true)
  ...
  test("A6: spill accounting includes carried leaf pointers in fixedRootCount") {
    withSQLConf(DeltaSQLConf.AMT_ENTRIES_PER_LEAF.key -> "2") {
      withTables() { (baselineDeltaLog, amtDeltaLog) =>
        ...
        val actual = checkpointAMTTable(amtDeltaLog, incremental = true)
  ...
  test("D8: writeIncremental rejects intermediate commits with a hole up to attemptVersion") {
    withSQLConf(leafPackingConfs: _*) {
      withTables() { (baselineDeltaLog, amtDeltaLog) =>
        ...
        fullCheckpoint(amtDeltaLog) // establishes a base AMT to build intermediate commits on.
        val snapshot = amtDeltaLog.update()
  ...
  test("D10: replay re-derives the root across a chain of incremental AMTs") {
    withSQLConf(leafPackingConfs: _*) {
      withTables() { (baselineDeltaLog, amtDeltaLog) =>
        ...
        checkpointAMTTable(amtDeltaLog, incremental = true)
        assertLiveAddFilesEquals(baselineDeltaLog, amtDeltaLog)
      }
    }
  }
