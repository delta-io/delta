...
case class IdentityColumnSpec(
  ...
  override def ddl: String = {
    val generatedAs = generatedAsIdentityType match {
      case GeneratedAsIdentityType.GeneratedAlways => "GENERATED ALWAYS"
      case GeneratedAsIdentityType.GeneratedByDefault => "GENERATED BY DEFAULT"
    }
    val identityOptions = Seq(
      startsWith.map(start => s"START WITH $start"),
      incrementBy.map(step => s"INCREMENT BY $step")).flatten
    val identityOptionsClause =
      if (identityOptions.isEmpty) "" else s" (${identityOptions.mkString(" ")})"
    val nullabilityClause = if (nullable) "" else " NOT NULL"
    val commentClause = comment.map(c => s" COMMENT '$c'").getOrElse("")
    s"$colName ${dataType.sql}$nullabilityClause $generatedAs AS IDENTITY" +
      s"$identityOptionsClause$commentClause"
  }
