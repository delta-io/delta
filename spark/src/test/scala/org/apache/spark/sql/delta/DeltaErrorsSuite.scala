  ...
  test("test DeltaErrors methods -- part 1") {
    ...
    }
    {
      ...
      checkError(e, "DELTA_CREATE_EXTERNAL_TABLE_WITHOUT_TXN_LOG", "42K03", Map(
        ...
        "tableName" -> "someTableName",
        "docLink" -> DeltaErrors
          .generateDocsLink(spark.sparkContext.getConf, "/index.html", skipValidation = true)
      ))
    }
    {
      val e = intercept[DeltaAnalysisException] {
    ...
    }
    {
      ...
      checkError(e, "DELTA_INCORRECT_LOG_STORE_IMPLEMENTATION", "0AKDC", Map(
        "docLink" -> DeltaErrors
          .generateDocsLink(sparkConf, "/delta-storage.html", skipValidation = true)
      ))
    }
    {
      val e = intercept[DeltaIllegalStateException] {
    ...
    }
    {
      ...
      checkError(e, "DELTA_INVALID_MANAGED_TABLE_SYNTAX_NO_SCHEMA", "42000", Map(
        "tableName" -> "table-1",
        "docLink" -> generateDocsLink(spark.sparkContext.getConf, "/index.html", skipValidation = true)
      ))
    }
    {
      val e = intercept[DeltaAnalysisException] {
  ...
  test("test DeltaErrors OSS methods more") {
    ...
    }
    {
      ...
      checkError(e, "DELTA_STREAMING_INCOMPATIBLE_SCHEMA_CHANGE_USE_SCHEMA_LOG", "42KD4", Map(
        "docLink" -> generateDocsLink(spark.sparkContext.getConf, docLink, skipValidation = true),
