...
class FileSizeHistogramSuite extends QueryTest
  ...
  override protected def sparkConf: SparkConf = {
    ...
    super.sparkConf.set(
      DeltaSQLConf.DELTA_ALL_FILES_IN_CRC_FORCE_VERIFICATION_MODE_FOR_NON_UTC_ENABLED.key, "false")
  }
  ...
  test("backward compatibility - deserialize FileSizeHistogram from old JSON") {
    // This test ensures that a VersionChecksum written by an older writer
    // can be deserialized by a newer reader.
    // FileSizeHistogram is part of Delta protocol (CRC), so this is critical.

    // Simulate JSON from an old writer
    val oldWriterJson =
      """{
        |  "txnId": "test-txn-id",
        |  "tableSizeBytes": 1000,
        ...
        |    "fileCounts": [1, 1, 1, 0, 0, 0],
        |    "totalBytes": [500, 5000, 50000, 0, 0, 0]
        |  },
        |  "deletedRecordCountsHistogramOpt": null,
        |  "allFiles": null
        |}""".stripMargin

    // Deserialize - tests FileSizeHistogram can still be read from JSON
    // written before the trait was added
    ...
    assert(histogram.fileCounts.toSeq === Seq(1L, 1L, 1L, 0L, 0L, 0L))
    assert(histogram.totalBytes.toSeq === Seq(500L, 5000L, 50000L, 0L, 0L, 0L))

    // Verify histogram operations work correctly
    assert(histogram.hashCode() != 0)
