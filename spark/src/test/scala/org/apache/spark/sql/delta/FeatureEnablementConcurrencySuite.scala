...
class FeatureEnablementConcurrencySuite
  ...
  ///////////////////////////////////////////////////////////////////
  // V2CheckpointTableFeature AFE: conflict resolution tests.      //
  ///////////////////////////////////////////////////////////////////
  ...
  test("v2Checkpoint AFE win: concurrent DML rebases successfully and writes V2 checkpoint") {
    val (deltaLog, _) = createTestTable(
  ...
  // -----------------------------------------------------------------------
  // Checkpoint format invariant tests -- all code paths x all scenarios
  //
  // These tests directly unit-test the three checkpoint-writing code paths,
  // independently of the DML end-to-end behavior tested by the AFE-win and
  // concurrent-scenarios tests above.
  //
  // Three code paths can write a checkpoint after a commit, all converging
  // on writeCheckpointFiles(snapshot):
  //
  //   Path 1: checkpoint(snapshot)        -- caller provides the snapshot
  //   Path 2: createCheckpointAtVersion   -- calls getSnapshotAt(v) internally
  //   Path 3: CheckpointHook (post-commit)-- calls getSnapshotAt(committedVersion),
  //                                          then delegates to Path 1
  //
  // Each test below exercises all three paths for one boundary scenario.
  // checkpointInterval=1 forces CheckpointHook to fire on every commit; the
  // explicit createCheckpointAtVersion and checkpoint(snapshot) calls then
  // exercise Paths 2 and 1 for the same version.
  //
  // V2 checkpoints use UUID-keyed filenames ({version}.checkpoint.{uuid}.parquet),
  // so multiple writes at the same version accumulate separate files rather than
  // overwriting. assertCheckpointFormat checks files.forall(format == expected),
  // which holds correctly regardless of how many files accumulate.
  //
  // Coverage:
  //
  //   Checkpoint timing                    | Path 1 | Path 2 | Path 3 (Hook)
  //   -------------------------------------|--------|--------|---------------
  //   Before AFE -> V1                     | (a)    | (a)    | (a)
  //   At AFE -> V2                         | (b)    | (b)    | (b)
  //   After AFE -> V2 (incl. rebased DML)  | (c)    | (c)    | (c)
  // -----------------------------------------------------------------------
  test("Checkpoint format invariant (a): all paths write V1 before v2Checkpoint AFE") {
    val (deltaLog, _) = createTestTable(
      properties = Seq(s"'${DeltaConfigs.CHECKPOINT_INTERVAL.key}' = '1'"))
    sql(s"DELETE FROM $testTableName WHERE idCol = 90")
    val preAfeVersion = deltaLog.update().version
    assert(preAfeVersion === 2)
    checkAnswer(
      spark.table(testTableName).select("idCol"),
      (0L until 100L).filter(_ != 90L).map(Row(_)))
    // Path 3 (CheckpointHook): fired automatically on the DML commit above.
    deltaLog.createCheckpointAtVersion(preAfeVersion) // Path 2
    deltaLog.checkpoint(deltaLog.update())             // Path 1
    assertCheckpointFormat(deltaLog, preAfeVersion, isV2 = false)
  }

  test("Checkpoint format invariant (b): all paths write V2 at v2Checkpoint AFE version") {
    val (deltaLog, _) = createTestTable(
      properties = Seq(s"'${DeltaConfigs.CHECKPOINT_INTERVAL.key}' = '1'"))
    sql(s"ALTER TABLE $testTableName SET TBLPROPERTIES " +
      s"('${DeltaConfigs.CHECKPOINT_POLICY.key}' = '${CheckpointPolicy.V2.name}')")
    val afeVersion = deltaLog.update().version
    assert(afeVersion === 2)
    // Path 3 (CheckpointHook): fired automatically on the AFE commit above.
    deltaLog.createCheckpointAtVersion(afeVersion) // Path 2
    deltaLog.checkpoint(deltaLog.update())          // Path 1
    assertCheckpointFormat(deltaLog, afeVersion, isV2 = true)
  }

  // Also covers the "Concurrent DML -> V2" matrix row: interleave makes the DML rebase
  // over the AFE commit. Both "simple DML after AFE" and "rebased concurrent DML" commit
  // at V > M and must produce V2 checkpoints via all paths.
  test("Checkpoint format invariant (c): all paths write V2 after v2Checkpoint AFE " +
    ...
    val v2CkptEnablementTxn = AlterTableProperty(
      property = DeltaConfigs.CHECKPOINT_POLICY.key, value = CheckpointPolicy.V2.name)
    val businessTxn = Delete(rows = Seq(90L))
    // AFE wins concurrently; DML rebases and commits at afeVersion+1.
    // Path 3 (CheckpointHook): fires automatically at afeVersion+1 (interval=1).
    businessTxn.interleave(ctx) { v2CkptEnablementTxn.execute(ctx) }
    ...
    checkAnswer(
      spark.table(testTableName).select("idCol"),
      (0L until 100L).filter(_ != 90L).map(Row(_)))
    deltaLog.createCheckpointAtVersion(dmlVersion) // Path 2
    deltaLog.checkpoint(deltaLog.update())          // Path 1
