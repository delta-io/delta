...
trait DeltaProtocolVersionSuiteBase extends QueryTest
  ...
  test("Deletion Vectors feature removal emits metrics") {
    val targetDF = spark.range(start = 0, end = 100, step = 1, numPartitions = 2).toDF()
    withTempDeltaTable(targetDF) { (targetTable, targetLog) =>
      ...
      val metrics = getUsageLogStats(usageLogs, "delta.deletionVectorsFeatureRemovalMetrics")
        .map(JsonUtils.fromJson[DeletionVectorsRemovalMetrics]).head

      assert(metrics.numDeletionVectorsToRemove === 1)
      assert(metrics.numDeletionVectorRowsToRemove === 10)
      assert(metrics.downgradeTimeMs > 0)
    }
  }
