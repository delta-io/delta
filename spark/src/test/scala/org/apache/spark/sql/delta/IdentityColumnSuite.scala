...
class IdentityColumnScalaSuite
  ...
  test("SQL DDL supports identity and generated columns for non-UC tables") {
    val tblName = getRandomTableName
    withTable(tblName) {
      Seq("CREATE TABLE", "REPLACE TABLE", "CREATE OR REPLACE TABLE").foreach { operation =>
        sql(
          s"""
             |$operation $tblName (
             |  id BIGINT GENERATED ALWAYS AS IDENTITY (START WITH 10 INCREMENT BY 2),
             |  value INT,
             |  generated INT GENERATED ALWAYS AS (value + 1)
             |) USING delta
             |""".stripMargin)
        sql(s"INSERT INTO $tblName (value) VALUES (3)")
        checkAnswer(sql(s"SELECT id, value, generated FROM $tblName"), Row(10L, 3, 4))
      }
    }
  }

  test("identity column DDL") {
    assert(IdentityColumnSpec(GeneratedAlways).ddl ===
      "id BIGINT GENERATED ALWAYS AS IDENTITY")
    assert(IdentityColumnSpec(GeneratedAlways, startsWith = Some(5)).ddl ===
      "id BIGINT GENERATED ALWAYS AS IDENTITY (START WITH 5)")
    assert(IdentityColumnSpec(GeneratedAlways, incrementBy = Some(-2)).ddl ===
      "id BIGINT GENERATED ALWAYS AS IDENTITY (INCREMENT BY -2)")
    assert(IdentityColumnSpec(
      GeneratedByDefault,
      startsWith = Some(-1),
      incrementBy = Some(2),
      colName = "custom_id",
      ...
      nullable = false).ddl ===
      "custom_id BIGINT NOT NULL GENERATED BY DEFAULT AS IDENTITY " +
        "(START WITH -1 INCREMENT BY 2) COMMENT 'identifier'")
  }
...
class IdentityColumnSqlSuite
  extends IdentityColumnSuiteBase
  with SQLDDLTestUtils

class IdentityColumnSqlIdColumnMappingSuite
  extends IdentityColumnSuiteBase
  with SQLDDLTestUtils
  with DeltaColumnMappingEnableIdMode {
...
class IdentityColumnSqlNameColumnMappingSuite
  extends IdentityColumnSuiteBase
  with SQLDDLTestUtils
  with DeltaColumnMappingEnableNameMode {
