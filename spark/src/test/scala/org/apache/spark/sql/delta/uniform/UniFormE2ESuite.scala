    ...
    withTable(testTableName) {
      write(
        s"""CREATE TABLE `$testTableName` (col1 INT) USING DELTA
           ...
           |  'delta.universalFormat.enabledFormats' = 'iceberg'
           |)""".stripMargin)
      write(s"INSERT INTO `$testTableName` VALUES (321),(654),(765),(133)")

      write(
        s"""CREATE OR REPLACE TABLE `$testTableName` (col2 INT) USING DELTA
           |TBLPROPERTIES (
           |  'delta.columnMapping.mode' = 'name',
           |  'delta.enableIcebergCompatV1' = 'true',
           |  'delta.universalFormat.enabledFormats' = 'iceberg'
      ...
      assert(read(s"SELECT col2 FROM $testTableName").toSeq ===
               Array(Row(123), Row(456), Row(567), Row(331)))
    }
  }
}
