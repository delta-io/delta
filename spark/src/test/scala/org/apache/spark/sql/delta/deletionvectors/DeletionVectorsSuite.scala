...
object DeletionVectorsSuite {
  ...
  // A template table with special characters in DV path. To use this table, one must use
  // the `withTableHavingSpecialCharInDVPath` method to create a copy of this table with paths
  // filled.
  ...
  def writeTableHavingSpecialCharInDVPath(path: File, pathIsEncoded: Boolean): Unit = {
    ...
    val fullPath = new File(
      path,
      if (pathIsEncoded) "folder%26with%25special%20char" else "folder&with%special char")
      .getCanonicalPath
