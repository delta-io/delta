...
class OptimizedWritesWithCustomShuffleManagerSuite extends OptimizedWritesSuiteBase
  ...
  writeTest("auto-detection - non-default shuffle manager uses getReader() without any conf") {
    dir =>
      ...
      // Whole-reducer bins now read the full partition (endMapIndex = Int.MaxValue) so that
      // a remote shuffle service does not have to sort the partition file, so boundedReads
      // is no longer the right discriminator here -- only genuinely split reducers are
      // bounded, which "map-range reads target single reducers with disjoint ranges" covers.
      // The authoritative signal that the writer took the getReader() path is the planned
      // event asserted just below.
      assert(RecordingDelegatingShuffleManager.recordedReads.asScala.nonEmpty,
