...
abstract class CommitCoordinatorSuiteBase
  ...
  // Test for ES-1617946: Enabling partitionOverwriteMode from OSS Spark on a CCv2 managed table
  // should not drop the table.
  test("saveAsTable with dynamic partition overwrite should not drop CCv2 managed table") {
    registerBuilder(TrackingInMemoryCommitCoordinatorBuilder(1))
    ...
    withSQLConf(DeltaSQLConf.DYNAMIC_PARTITION_OVERWRITE_ENABLED.key -> "true") {
      withTempDir { tempDir =>
        ...
        // Step 1: Create a partitioned table with coordinated commits enabled
        Seq((1, "2024-01-01", 10), (2, "2024-01-01", 11), (3, "2024-01-01", 12), (4, "2024-01-02", 10))
          .toDF("value", "time_date_level", "time_hour_level")
          .write
          .format("delta")
        ...
        val logInitial = DeltaLog.forTable(spark, tablePath)
        assert(
          logInitial.unsafeVolatileSnapshot.tableCommitCoordinatorClientOpt.nonEmpty ||
            logInitial.unsafeVolatileSnapshot.isCatalogOwned,
          "Table should have coordinated commits enabled after creation")

        // Verify initial data
        val initialCount = spark.read.format("delta").load(tablePath).count()
        assert(initialCount == 4, s"Expected 4 rows, got $initialCount")

        // Step 2: Read data from a specific partition and write back using dynamic partition
        // overwrite mode
        // This is the operation that causes the bug - it should NOT drop the table
        val dfToWrite = spark.read.format("delta").load(tablePath)
          .filter("time_hour_level = 12")
          .limit(2)
        ...
        // Step 3: Verify the table still exists and has the correct data
        // With dynamic partition overwrite, only partition time_hour_level=12 should be affected
        // Other partitions should remain intact
        val finalData = spark.read.format("delta").load(tablePath)
        val finalCount = finalData.count()

        // Expected: rows from partitions (2024-01-01, 10), (2024-01-01, 11), (2024-01-02, 10)
        // should be preserved, plus the overwritten partition (2024-01-01, 12)
        assert(finalCount >= 3,
          s"Expected at least 3 rows (from unaffected partitions), got $finalCount")

        // Verify data from unaffected partitions is preserved
        val hour10Count = finalData.filter("time_hour_level = 10").count()
        val hour11Count = finalData.filter("time_hour_level = 11").count()
        assert(hour10Count == 2,
          s"Partition time_hour_level=10 should have 2 rows, got $hour10Count")
        assert(hour11Count == 1,
          s"Partition time_hour_level=11 should have 1 row, got $hour11Count")

        // Verify the CCv2/CatalogOwned feature is still enabled
        val log = DeltaLog.forTable(spark, tablePath)
        assert(
          log.unsafeVolatileSnapshot.tableCommitCoordinatorClientOpt.nonEmpty ||
            log.unsafeVolatileSnapshot.isCatalogOwned,
          "Table should still have coordinated commits enabled")
      }
    }
  }

  // Additional test: saveAsTable with dynamic partition overwrite using DataFrame API with
  // a catalog table reference. This more closely matches the original bug report scenario
  // from ES-1617946 where the table was registered in a catalog.
  test("DataFrame saveAsTable with dynamic partition overwrite should preserve CCv2 table") {
    registerBuilder(TrackingInMemoryCommitCoordinatorBuilder(1))
    ...
    withSQLConf(DeltaSQLConf.DYNAMIC_PARTITION_OVERWRITE_ENABLED.key -> "true") {
      withTempDir { tempDir =>
        ...
        // Verify coordinated commits is enabled
        val logBefore = DeltaLog.forTable(spark, tablePath)
        assert(
          logBefore.unsafeVolatileSnapshot.tableCommitCoordinatorClientOpt.nonEmpty ||
            logBefore.unsafeVolatileSnapshot.isCatalogOwned,
          "Table should have coordinated commits enabled")

        val initialCount = spark.read.format("delta").load(tablePath).count()
        assert(initialCount == 4, s"Expected 4 rows, got $initialCount")

        // Step 2: Read subset of data from partition 'x'
        val dataToOverwrite = spark.read.format("delta").load(tablePath)
          .filter("part = 'x'")
          .limit(1)

        // Step 3: Perform dynamic partition overwrite - this should NOT drop the table
        // This is the exact scenario that triggers the bug in ES-1617946
        dataToOverwrite
          .write
          .format("delta")
          .mode("overwrite")
        ...
        // Step 4: Verify table still exists and data integrity
        val finalData = spark.read.format("delta").load(tablePath)

        // Verify partition 'y' data is preserved (dynamic overwrite should only affect 'x')
        val partYCount = finalData.filter("part = 'y'").count()
        assert(partYCount == 2,
          s"Partition 'y' should have 2 rows preserved, got $partYCount")

        // Verify partition 'x' was overwritten (should have 1 row now due to limit(1))
        val partXCount = finalData.filter("part = 'x'").count()
        assert(partXCount == 1,
          s"Partition 'x' should have 1 row after overwrite, got $partXCount")

        // Verify coordinated commits is still enabled
        val logAfter = DeltaLog.forTable(spark, tablePath)
        assert(
          logAfter.unsafeVolatileSnapshot.tableCommitCoordinatorClientOpt.nonEmpty ||
            logAfter.unsafeVolatileSnapshot.isCatalogOwned,
          "Table should still have coordinated commits enabled after overwrite")
      }
    }
  }
}
