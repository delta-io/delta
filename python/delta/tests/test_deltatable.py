...
class DeltaTableTests(DeltaTestCase):
    ...
    def __verify_table_schema(
        ...
        tableDetails = self.spark.sql("DESCRIBE DETAIL {}".format(tableName)).collect()[
            0
        ]
    ...
    def test_create_table_behavior(self) -> None:
        ...
        self.__verify_table_schema(
            "testTable", schema, ["c1"], [IntegerType()], nullables={"c1"}
        )
    ...
    def test_replace_table_with_name_and_location(self) -> None:
        for orCreate in (False, True):
            ...
            tableName = "testTable{}".format(orCreate)
            self.spark.sql(
                "CREATE TABLE {} (col int) USING DELTA LOCATION '{}'".format(
                    tableName, path
                )
            )
