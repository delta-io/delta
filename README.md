...
#### JDK Setup
Build needs JDK 17. Make sure to setup `JAVA_HOME` that points to JDK 17.
...
4. Under `Project JDK` specify a valid Java `17` JDK and opt to use SBT shell for `project reload` and `builds`.
5. Click `Finish`.
6. In your terminal, run `build/sbt clean package`. Make sure you use Java `17`. The build will generate files 
...
1. Ensure you are using Java `17`. You can set this using
```
