...
class UCLegacyLoadTableAdapterSuite extends AnyFunSuite {
  ...
  private def dataTypeJsonColumn(
      name: String,
      typeJson: String,
      nullable: Boolean = true): ColumnInfo = new ColumnInfo()
    .name(name)
    .typeJson(typeJson)
    .nullable(nullable)
    .position(0)
  ...
  test("legacy fallback rejects a missing column list instead of inventing an empty schema") {
    val e = intercept[java.io.IOException] {
      UCLegacyLoadTableAdapter.loadTableViaLegacyApi(
        new StubTablesApi(baseTableInfo.columns(null)),
        "main",
        "default",
        "tbl")
    }
  ...
  test("legacy fallback rejects invalid partition indexes") {
    ...
    assert(e.getMessage.contains("invalid partition_index"))
  }

  test("legacy fallback rejects negative partition indexes") {
  ...
  test("legacy fallback rejects non-contiguous partition indexes") {
    val tableInfo = baseTableInfo
      .columns(
        Seq(
          primitiveColumn("date", "date").partitionIndex(0),
          primitiveColumn("region", "string").partitionIndex(2)).asJava)
  ...
  test("legacy fallback accepts data-type-only type_json from older Delta writers") {
    val tableInfo = baseTableInfo.columns(
      Seq(
        dataTypeJsonColumn("primitive", "\"string\""),
        dataTypeJsonColumn(
          "arr",
        ...
        dataTypeJsonColumn(
          "struct_col",
          """{"type":"struct","fields":[{"name":"nested","type":"long","nullable":false,"metadata":{}}]}""",
          nullable = false)).asJava)

    val metadata = UCLegacyLoadTableAdapter
      .loadTableViaLegacyApi(new StubTablesApi(tableInfo), "main", "default", "tbl")
      .getMetadata

    val fields = metadata.getColumns.getFields
    val arrType = fields.get(1).getType.asInstanceOf[model.ArrayType]
    val mapType = fields.get(2).getType.asInstanceOf[model.MapType]
    val structType = fields.get(3).getType.asInstanceOf[model.StructType]

    assert(fields.get(0).getName === "primitive")
    assert(fields.get(0).getType.asInstanceOf[model.PrimitiveType].getType === "string")
    assert(fields.get(3).getName === "struct_col")
    assert(!fields.get(3).getNullable)
    assert(arrType.getContainsNull)
    assert(arrType.getElementType.asInstanceOf[model.PrimitiveType].getType === "string")
    assert(mapType.getKeyType.asInstanceOf[model.PrimitiveType].getType === "string")
    assert(mapType.getValueType.asInstanceOf[model.PrimitiveType].getType === "long")
    assert(!mapType.getValueContainsNull)
    assert(structType.getFields.get(0).getName === "nested")
    assert(!structType.getFields.get(0).getNullable)
  }
