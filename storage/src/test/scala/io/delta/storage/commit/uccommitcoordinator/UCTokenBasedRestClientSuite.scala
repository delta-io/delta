...
class UCTokenBasedRestClientSuite
  ...
  test("commit with uniform.iceberg sends correct snake_case JSON per all.yaml") {
    val cases = Seq(
      // (description, IcebergMetadata, expectedBaseVersion)
      (
        "without baseConvertedDeltaVersion",
        new IcebergMetadata("s3://bucket/metadata/v1.json", 42L, "2025-01-04T03:13:11.423Z"),
        None
      ),
      (
        "with baseConvertedDeltaVersion",
        new IcebergMetadata(
          "s3://bucket/metadata/v2.json", 43L, "2025-01-05T00:00:00.000Z",
          Optional.of(java.lang.Long.valueOf(42L))),
        Some(42L)
      )
    )

    cases.foreach { case (desc, icebergMeta, expectedBase) =>
      ...
      assert(iceberg.get("metadata_location").asText() === icebergMeta.getMetadataLocation, desc)
      assert(iceberg.get("converted_delta_version").asLong() ===
        icebergMeta.getConvertedDeltaVersion, desc)
      assert(iceberg.get("converted_delta_timestamp").asText() ===
        icebergMeta.getConvertedDeltaTimestamp, desc)
