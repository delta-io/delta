 ...
 * from FileSink.
 */
public class DeltaSink<IN> implements Sink<IN, AbstractDeltaCommittable,
     ...
     * is taken from the {@link DeltaSinkBuilder#getAppId} what guarantees us that each writer will
     * get the same value. In other case, if we are provided by the Flink framework with some
     * previous writers' states then we use those to restore values of appId and nextCheckpointId.
     *
