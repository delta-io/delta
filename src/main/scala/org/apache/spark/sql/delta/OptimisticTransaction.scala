...
import org.apache.spark.sql.delta.hooks.{GenerateManifestImpl, PostCommitHook, GenerateSymlinkManifest, GenerateJsonManifest}
    ...
    val version = try {
      ...
      if (hasFileActions) {
        ...
        if (DeltaConfigs.JSON_FORMAT_MANIFEST_ENABLED.fromMetaData(metadata)) {
          registerPostCommitHook(GenerateJsonManifest)
        }
      }
