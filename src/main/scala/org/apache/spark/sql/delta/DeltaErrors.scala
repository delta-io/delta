...
trait DocsPath {
  /**
   * The URL for the base path of Delta Lake's docs.
   */
...
object DeltaErrors
  ...
  def operationNotSupportedException(
    ...
    new AnalysisException(
        s"Operation not allowed: `$operation` is not supported " +
          s"for Delta Lake tables: $tableIdentifier")
  }

  def operationNotSupportedException(operation: String): Throwable = {
    new AnalysisException(
      s"Operation not allowed: `$operation` is not supported for Delta Lake tables")
  }

  def emptyDataException: Throwable = {
    new AnalysisException(
      "Data used in creating the Delta Lake table doesn't have any columns.")
  }

  def notADeltaTableException(deltaTableIdentifier: DeltaTableIdentifier): Throwable = {
    new AnalysisException(s"$deltaTableIdentifier is not a Delta Lake table.")
  }

  def notADeltaTableException(
      operation: String, deltaTableIdentifier: DeltaTableIdentifier): Throwable = {
    new AnalysisException(s"$deltaTableIdentifier is not a Delta Lake table. " +
      s"$operation is only supported for Delta Lake tables.")
  }

  def notADeltaTableException(operation: String): Throwable = {
    new AnalysisException(s"$operation is only supported for Delta Lake tables.")
  }
  ...
  def vacuumBasePathMissingException(baseDeltaPath: Path): Throwable = {
    new AnalysisException(
      s"Please provide the base path ($baseDeltaPath) when Vacuuming Delta Lake tables. " +
  ...
  def useOtherFormatOnDeltaPathException(
    ...
    new AnalysisException(
      s"""Incompatible format detected.
        ...
        |'format("delta")' when reading and writing to a Delta Lake table.
        |
        |To disable this check, SET spark.databricks.delta.formatCheck.enabled=false
        |To learn more about Delta, see ${baseDocsPath(spark)}/delta/index.html
        |""".stripMargin)
  ...
  def logFileNotFoundExceptionForStreamingSource(e: FileNotFoundException): Throwable = {
    new FileNotFoundException(e.getMessage + " If you never deleted it, it's " +
      ...
      "policy of your Delta Lake table").initCause(e)
  }
  ...
  def multipleLoadPathsException(paths: Seq[String]): Throwable = {
    throw new AnalysisException(
      s"""
        ...
        |multiple paths from the same Delta Lake table, please load the root path of
        |the Delta Lake table with the corresponding partition filters. If the multiple paths
        |are from different Delta Lake tables, please use Dataset's union()/unionByName() APIs
  ...
  def truncateTablePartitionNotSupportedException: Throwable = {
    new AnalysisException(
      s"Operation not allowed: TRUNCATE TABLE on Delta Lake tables does not support" +
  ...
  def multipleSourceRowMatchingTargetRowInMergeException: Throwable = {
    new UnsupportedOperationException("Cannot perform MERGE as multiple source rows " +
        "matched and attempted to update the same target row in the Delta Lake table.")
  }
  ...
  def versionNotExistException(userVersion: Long, earliest: Long, latest: Long): Throwable = {
    throw new AnalysisException(s"Cannot time travel Delta Lake table to version $userVersion. " +
...
/**
 * Thrown when the metadata of the Delta Lake table has changed between the time of read
 * and the time of commit.
 */
class MetadataChangedException(
    conflictingCommit: Option[CommitInfo]) extends DeltaConcurrentModificationException(
  "The metadata of the Delta Lake table has been changed by a concurrent update. " +
...
class ProtocolChangedException(
    conflictingCommit: Option[CommitInfo]) extends DeltaConcurrentModificationException(
  "The protocol version of the Delta Lake table has been changed by a concurrent update. " +
    "Please try the operation again.", conflictingCommit)
...
class MetadataMismatchErrorBuilder {
  ...
  def addSchemaMismatch(original: StructType, data: StructType): Unit = {
    bits ++=
      s"""A schema mismatch detected when writing to the Delta Lake table.
