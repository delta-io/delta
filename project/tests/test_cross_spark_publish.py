...
NOTE: This script reads Spark version information from target/spark-versions.json,
which is generated by: build/sbt exportSparkVersionsJson
The JSON file is the single source of truth for Spark versions, derived from
...
# Spark versions are loaded dynamically from target/spark-versions.json
# This is generated by: build/sbt exportSparkVersionsJson
# DO NOT hardcode Spark versions here - they are the single source of truth in CrossSparkVersions.scala
def load_spark_versions() -> tuple[Dict[str, SparkVersionSpec], str]:
    """
    Load Spark version information from the JSON file generated by SBT.
    
    Returns:
        tuple: (SPARK_VERSIONS dict, DEFAULT_SPARK string)
    """
    # Find the JSON file
    ...
    if not json_path.exists():
        print(f"ERROR: Spark versions JSON not found at {json_path}")
        print("Please run: build/sbt exportSparkVersionsJson")
        sys.exit(1)
    ...
    # Build SPARK_VERSIONS dict
    spark_versions = {}
    for version_info in data["allVersions"]:
        full_version = version_info["fullVersion"]
        suffix = version_info["suffix"]
        spark_versions[full_version] = SparkVersionSpec(suffix)
    
    # Get default version
    default_spark = data["default"]
    
    return spark_versions, default_spark

# Load Spark versions from JSON
SPARK_VERSIONS, DEFAULT_SPARK = load_spark_versions()
...
class CrossSparkPublishTest:
    ...
    def validate_spark_versions(self) -> None:
        ...
        Since we now load versions from the JSON file generated by SBT, this validation
        is simpler - we just need to ensure the JSON file is up-to-date.
        """
        # The versions are loaded from JSON, so they are automatically in sync
        # We just print them for visibility
        print(f"✓ Spark versions loaded from JSON: {', '.join(sorted(SPARK_VERSIONS.keys()))}")
        print(f"✓ Default Spark version: {DEFAULT_SPARK}\n")
