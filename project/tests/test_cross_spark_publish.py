...
class CrossSparkPublishTest:
    ...
    def validate_spark_versions(self) -> None:
        ...
        try:
            ...
            # Version format: X.Y.Z, X.Y.Z-SNAPSHOT, X.Y.Z-previewN, X.Y.Z-rcN
            import re
            version_pattern = re.compile(r'^\d+\.\d+\.\d+(-(SNAPSHOT|preview\d+|rc\d+))?$')
...
class SparkVersionsScriptTest:
    ...
    def test_released_spark_versions(self) -> bool:
        ...
        try:
            ...
            # Load JSON and verify any version with a `-` suffix is excluded
            # (covers `-SNAPSHOT`, `-previewN`, `-rcN`, etc.).
            with open(self.json_path, 'r') as f:
                data = json.load(f)

            expected_count = sum(1 for entry in data if "-" not in entry["fullVersion"])
