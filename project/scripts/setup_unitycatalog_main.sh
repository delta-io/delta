...
UC_PIN_SHA=d0f52b50afa9167c9916db1067089f4526f4a849
UC_PIN_FETCH_REF=pull/1856/head
...
if [[ "$UC_REF" == "$UC_PIN_SHA" && -n "$UC_PIN_FETCH_REF" ]]; then
  ...
  FETCHED_SHA="$(git -C "$UC_DIR" rev-parse FETCH_HEAD)"
  if [[ "$FETCHED_SHA" != "$UC_PIN_SHA" ]]; then
    echo "ERROR: $UC_PIN_FETCH_REF resolved to $FETCHED_SHA, expected UC_PIN_SHA=$UC_PIN_SHA." >&2
    exit 1
  fi
...
if [[ "$DELTA_RELEASE_MODE" != "1" && "$UC_REF" == "$UC_PIN_SHA" && -z "$UC_PIN_FETCH_REF" ]]; then
...
elif [[ "$UC_REF" == "$UC_PIN_SHA" && -z "$UC_PIN_FETCH_REF" ]]; then
