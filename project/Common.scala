...
/**
 * Common build settings, dependencies, and helper functions shared across all Delta projects.
 */
object Common {
  ...
  /**
   * Run a task only on Spark master version, skip on other versions.
  ...
  /**
   * Get Scala parallel collections dependency for Scala 2.13+.
   */
  ...
  /**
   * Get scala binary version for sqlDeltaImport (doesn't support 2.11).
   */
  ...
  lazy val releaseSettings = Seq(
    ...
    PgpKeys.pgpPassphrase := sys.env.get("PGP_PASSPHRASE").map(_.toArray),
