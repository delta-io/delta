...
That means specifically that for any commit…
 ...
 - it is **illegal** for a `path` to occur in an `add` action that already occurs with a different `dvId` in the list of `add` actions from the snapshot of the version immediately preceding the commit, unless the commit also contains a remove for the later combination.
