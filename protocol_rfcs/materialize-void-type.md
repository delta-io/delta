...
A `void` column in any other position is non-structural and must be omitted. In particular, a `void` partition column is never structural. A schema with one of the shapes above **requires** the `materializeVoidType` feature.

###### Writer Requirements for Materialize Void Type

When Materialize Void Type is supported (when the `writerFeatures` field of a table's `protocol` action contains `materializeVoidType`), writers:
...
###### Reader Requirements for Materialize Void Type

When Materialize Void Type is supported (when the `readerFeatures` field of a table's `protocol` action contains `materializeVoidType`), readers:
...
###### Removing the Materialize Void Type feature
...
In the version that removes `materializeVoidType` from `writerFeatures` and `readerFeatures`, writers:
...
After `materializeVoidType` is removed, data files may still contain `UNKNOWN` columns that correspond to columns dropped from the current table schema. Readers should ignore these columns and process only columns referenced by the current table schema. Historical table versions that expose the dropped columns still advertise `materializeVoidType`.
...
When a structural `void` column is materialized, it must use the `UNKNOWN` logical type and the table must support `materializeVoidType`. Every non-structural `void` column must be omitted from the Parquet file.
