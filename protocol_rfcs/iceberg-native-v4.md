...
1. **Should the feature be strictly enable-only?** This RFC forbids flipping
   `delta.enableIcebergNativeV4` back to `false` outside feature removal, following IcebergCompatV3.
   Disabling in place would also be safe, since the tree stays valid and no data rewrite is needed.

2. **Is the loss of cheap concurrent appends acceptable?** If not, a bounded escape hatch could
   let a small append land as a log commit provided the next commit folds it in. That weakens the
   guarantee, since an Iceberg engine could momentarily read an incomplete table, so it would need a
   way for readers to detect the pending state.

3. **Should `id`-mode Column Mapping be required?** This RFC inherits IcebergCompatV3's allowance of
...
4. **Enablement on a large existing table.** Folding all pre-existing state in the first commit may
   be expensive. Staging it across several commits, with the guarantee taking effect once folding
   completes, may be preferable.
