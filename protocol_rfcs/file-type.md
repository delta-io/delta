...
**Field IDs.** The FILE sub-fields have reserved field IDs at fixed offsets from the `file` column's own field ID: `uri` = +1, `offset` = +2, `size` = +3, `content_type` = +4, `checksum` = +5, `inline` = +6. A `file` column therefore reserves seven consecutive field IDs (its own plus these six); a writer must advance the table's maximum column id (`delta.columnMapping.maxColumnId`) past all seven and must not assign these IDs to any other field. These offsets match the [Iceberg `FILE` type](https://github.com/apache/iceberg/pull/17918) so that field IDs align when the table is read as Iceberg or uses field-ID-keyed statistics.

> Note: the per-leaf addressing above describes the current per-file statistics format — `add.stats`, a JSON object keyed by physical name. When field-ID-keyed statistics are used (the in-flight [Iceberg V4 Adaptive Metadata Tree RFC](https://github.com/delta-io/delta/issues/6640)), each sub-field's statistics are keyed by its reserved field ID above.
